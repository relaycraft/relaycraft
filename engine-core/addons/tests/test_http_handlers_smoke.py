...
class TestHttpHandlersSmoke(unittest.TestCase):
    def test_relay_poll_success_and_exception(self):
        ...
        with patch("core.http_handlers.realtime.get_indices", return_value=[{"id": "f1", "msg_ts": 123}]):
        ...
        with patch("core.http_handlers.realtime.get_indices", side_effect=RuntimeError("db error")):
            flow = _make_flow(query={"since": "0"})
    ...
    def test_relay_detail_success_and_exception(self):
        with patch("core.http_handlers.realtime.get_detail", return_value={"id": "f1"}):
            monitor = _make_monitor()
            flow = _make_flow(query={"id": "f1"})
            handle_realtime_routes(monitor, flow, "relay_detail", _FakeResponse, _safe_json_default)
            self.assertEqual(flow.response.status_code, 200)

        with patch("core.http_handlers.realtime.get_detail", side_effect=RuntimeError("detail error")):
            monitor = _make_monitor()
            flow = _make_flow(query={"id": "f1"})
            handle_realtime_routes(monitor, flow, "relay_detail", _FakeResponse, _safe_json_default)
            self.assertEqual(flow.response.status_code, 500)
    ...
    def test_relay_search_success_and_exception(self):
        with patch("core.http_handlers.data.search_by_body", return_value={"matches": ["f1"], "scanned": 1}):
            monitor = _make_monitor()
    ...
    def test_relay_session_activate_success_and_exception(self):
        with patch("core.http_handlers.control.switch_session", return_value=True):
            monitor = _make_monitor()
        ...
        with patch("core.http_handlers.control.switch_session", side_effect=RuntimeError("activate error")):
            monitor = _make_monitor()
            flow = _make_flow(content=json.dumps({"id": "s1"}).encode("utf-8"), method="POST")
            handle_control_routes(monitor, flow, "relay_session_activate", _FakeResponse)
            self.assertEqual(flow.response.status_code, 500)
    ...
    def test_relay_session_delete_success_and_exception(self):
        with patch("core.http_handlers.control.delete_session", return_value=True):
            monitor = _make_monitor()
            flow = _make_flow(content=json.dumps({"id": "s1"}).encode("utf-8"), method="POST")
            handle_control_routes(monitor, flow, "relay_session_delete", _FakeResponse)
            self.assertEqual(flow.response.status_code, 200)

        with patch("core.http_handlers.control.delete_session", side_effect=RuntimeError("delete error")):
            monitor = _make_monitor()
            flow = _make_flow(content=json.dumps({"id": "s1"}).encode("utf-8"), method="POST")
            handle_control_routes(monitor, flow, "relay_session_delete", _FakeResponse)
            self.assertEqual(flow.response.status_code, 500)
    ...
    def test_relay_session_clear_success_and_exception(self):
        with patch("core.http_handlers.control.get_active_session", return_value={"id": "s_active"}):
            monitor = _make_monitor()
        ...
        with patch("core.http_handlers.control.clear_session", side_effect=RuntimeError("clear error")):
            monitor = _make_monitor()
            flow = _make_flow(content=b"", method="POST")
            handle_control_routes(monitor, flow, "relay_session_clear", _FakeResponse)
            self.assertEqual(flow.response.status_code, 500)
    ...
    def test_relay_import_session_success_and_exception(self):
        with patch("core.http_handlers.importers.create_session", return_value="s_imported"):
            monitor = _make_monitor()
        ...
        with patch("core.http_handlers.importers.create_session", side_effect=RuntimeError("import error")):
            monitor = _make_monitor()
