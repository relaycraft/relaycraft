...
class ActionExecutor:
    ...
    def apply_json_modification(self, data: Union[Dict, List], path: str, value: Any, operation: str) -> None:
        ...
        try:
            ...
            if operation == "set":
                ...
                # Group matches by parent ID to handle multiple deletions safely
                matches_by_parent = {}
                ...
                for pid, info in matches_by_parent.items():
                    ...
                    if isinstance(parent, list):
                        ...
                        for m in p_matches:
                            if isinstance(m.path, jsonpath_ng.Fields):
                                # match.path.fields is a tuple/list of field names
                ...
                # Append to array (only if match is an array)
                for match in matches:
    ...
    def apply_map_local(self, flow: http.HTTPFlow, action: Dict[str, Any], url_match: Optional[re.Match] = None) -> None:
        ...
        if file_path.exists() and file_path.is_file():
            ...
            # However, handle_request has already recorded the 'success' hit.
            # We need to find the rule object or ID.
    ...
    def apply_map_remote(self, flow: http.HTTPFlow, action: Dict[str, Any], url_match: Optional[re.Match] = None) -> None:
        ...
        # Check if regex substitution is intended ($1, \1, etc)
        use_regex_sub = False
        if url_match and target_url:
            # Check if target_url contains group references like $1 or \1
            ...
            if preserve_path:
                ...
                try:
                    ...
                    if parsed.port:
                        ...
                        # Reset port to default for the new scheme
                        flow.request.port = 443 if flow.request.scheme == "https" else 80
                    
                    # Update Path (Prepend target path if exists)
    ...
    def apply_throttle(self, flow: http.HTTPFlow, action: Dict[str, Any], phase: str = "request") -> None:
        ...
        # 1. Simulate Latency (Request phase only by default)
        if phase == "request" and delay_ms > 0:
            time.sleep(delay_ms / 1000.0)
            # ctx.log.info(f"Latency delay: {delay_ms}ms")
            
        # 2. Simulate Packet Loss (Request phase only to kill the connection early)
        ...
        if bandwidth_kbps > 0:
            ...
            if content:
                ...
                # Formula: delay_sec = (bytes * 8 bits) / (kbps * 1000 bps)
