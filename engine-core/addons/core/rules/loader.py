...
class RuleLoader:
    ...
    def load_rules(self) -> None:
        ...
        try:
            ...
            # 2. Fast Path: Check directory mtime first
            # (Adding/Deleting files usually updates the directory's own mtime)
            ...
            # If nothing changed since last check (dir mtime AND file count), skip heavy stat scan
            if dir_mtime <= self._last_load_time and current_file_count == self._last_file_count and self.rules:
                # Note: This doesn't catch content changes inside subdirectories if the dir mtime doesn't bubble up,
                # but for most flattening folder structures it is much faster.
                # To be 100% safe, we can still do a max_mtime check but less frequently.
                pass
        ...
        for rule in self.rules:
            ...
            for atom in match_req:
                ...
                if atom.get("type") == "host":
                    ...
                    if invert:
                        # Inverted restrictions cannot be indexed efficiently as a "positive" match
                        # They must be treated as complex/global to ensure they are checked
                    ...
                    if m_type == "exact":
                        ...
                        # If it's not a simple exact host, it goes to wildcard/complex index
                        has_host_restriction = "complex"
