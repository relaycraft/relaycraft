...
pub async fn plugin_install_local_zip(path: String, _app: AppHandle) -> Result<String, String> {
    ...
    let _ =
        logging::write_domain_log("audit", &format!("Installed Local Plugin/Theme: {}", id));
...
pub async fn read_theme_file(
    ...
    // [SECURITY] 1. Extension Allowlist
    // Only allow reading CSS files to prevent reading sensitive data or other formats
    ...
    // [SECURITY] 2. Basic Path Sanitization
    // Prevent obvious directory traversal attempts in the filename
    ...
    // We strictly assume the theme directory name matches the ID
    let theme_dir = themes_dir.join(&theme_id);

    // Verify theme directory exists and is valid
    if !theme_dir.exists() {
    ...
    // This handles symlinks and any other path manipulation tricks
