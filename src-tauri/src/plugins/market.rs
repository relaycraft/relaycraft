...
pub struct RegistryPlugin {
    ...
    // The market JSON has "url" for yaml/metadata and "downloadUrl" for the zip/rctheme
    // We want the download url for installation.
    // If the JSON provides both, we should capture them.

    // This maps to "url" in JSON (metadata url)
    pub url: Option<String>,

    // This maps to "downloadUrl" in JSON (the actual package)
...
pub async fn plugin_market_install(url: String) -> Result<String, String> {
    ...
    // 1. Download the .rcplugin (zip) to a temp file
    ...
    let client = Client::builder()
        .timeout(std::time::Duration::from_secs(300)) // Longer timeout for big files
    ...
    let resp = client
        .get(&url) // The frontend passes the correct url string directly
        .send()
        .await
