            ...
            <ReactMarkdown
                components={{
                    ...
                    code({ node, inline, className, children, ...props }: any) {
                        ...
                        return (
                            <code className="bg-primary/10 px-1.5 py-0.5 rounded text-[11px] font-mono text-primary font-bold mx-0.5" {...props}>
                                {children}
                            </code>
                        );
                    },
                    // Vertical Density: Tight paragraphs
                    p: ({ children }) => <div className="mb-2 leading-relaxed text-foreground/90 last:mb-0">{children}</div>,
                    // Compact Headers
                    h1: ({ children }) => <h1 className="text-base font-bold mb-3 mt-5 first:mt-0 text-foreground border-b border-border/20 pb-1.5">{children}</h1>,
                    ...
                    th: ({ children }) => <th className="px-3 py-1.5 font-bold uppercase text-[10px] tracking-wider opacity-60">{children}</th>,
