...
export function CopyButton({
    ...
    const variantClasses = {
        ...
        secondary: "bg-background/80 backdrop-blur rounded-lg border border-border/40 hover:bg-background h-8 w-8 justify-center",
        quiet: "border border-white/5 bg-transparent hover:bg-white/5 hover:border-white/20 h-8 w-8 justify-center transition-all duration-200"
    };
        ...
        <Tooltip content={copied ? t('copy_button.copied') : displayLabel} side={tooltipSide}>
            ...
            >
                <div className="relative">
                    ...
                    <Check
                        size={iconSize}
                        className={cn("text-green-500 absolute top-0 left-0 transition-all scale-0 opacity-0", copied && "scale-100 opacity-100")}
                    />
                </div>
                {showLabel && (
