import { ComponentProps, useState, useRef, useEffect, Children, isValidElement, ReactElement } from 'react';
...
export const Select = ({
    ...
    const options = propOptions || Children.map(children, child => {
        if (isValidElement(child) && child.type === 'option') {
            const optionChild = child as ReactElement<ComponentProps<'option'> & { 'data-trigger-label'?: string }>;
            return {
                ...
                disabled: optionChild.props.disabled || false
            };
        }
        return null;
    })?.filter(Boolean) || [];
        ...
        >
            ...
            >
                <span className="truncate text-left flex-1">
                    {selectedOption ? (selectedOption.triggerLabel || selectedOption.label) : (placeholder || 'Select...')}
                </span>
