...
export function FilterBar({
  ...
  const draftTrafficFilter = useUIStore((state) => state.draftTrafficFilter);
  const setDraftTrafficFilter = useUIStore((state) => state.setDraftTrafficFilter);

  useEffect(() => {
    if (draftTrafficFilter) {
      setFilterText(draftTrafficFilter);
      setDraftTrafficFilter(null);
    }
  }, [draftTrafficFilter, setFilterText, setDraftTrafficFilter]);
    ...
    <div className="p-3 flex-shrink-0 z-50 relative flex flex-col gap-2 bg-muted/15 backdrop-blur-xl border-b border-border/40">
      <div className="flex items-center gap-2">
        <div className="relative flex-1 group">
          {bodySearching ? (
            <Loader2 className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-primary animate-spin transition-colors z-10" />
          ) : (
        ...
        {/* Count Badge */}
        <div className="hidden sm:flex items-center px-2 py-1 bg-muted/30 border border-border/50 rounded-md text-xs font-mono text-muted-foreground whitespace-nowrap tabular-nums z-10">
            ...
            <div className="bg-muted/50 rounded-lg p-2 text-xs border border-border/50 text-muted-foreground">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1 opacity-80">
                <div>
                  ...
                  <span className="opacity-60 text-tiny">(e.g. method:POST type:json)</span>
                </div>
                <div>
                  • <b>{t("traffic.filter.or")}</b>: {t("traffic.filter.or_desc")}{" "}
                  <span className="opacity-60 text-tiny">(e.g. status:4xx status:5xx)</span>
                </div>
                <div>
                  • <b>{t("traffic.filter.neg")}</b>{" "}
                  <span className="opacity-60 text-tiny">(e.g. -domain:google)</span>
                </div>
                <div>
              ...
              <div className="flex items-start gap-1.5 mt-2 px-2 py-1.5 bg-primary/5 border border-primary/15 rounded-md text-xs text-muted-foreground">
                <Info className="w-3 h-3 mt-0.5 flex-shrink-0 text-primary/60" />
                <span>{t("traffic.filter.deep_search_notice")}</span>
              </div>
            </div>
          </motion.div>
        )}
