...
import { Download, FileText, Code, Eye, FileDigit } from 'lucide-react';
import { CopyButton } from '../common/CopyButton';
import { useTranslation } from 'react-i18next';
import { detectContentType, formatJson, formatXml, getExtensionFromHeaders } from '../../lib/contentUtils';
        ...
        if (contentType === 'binary' || encoding === 'base64') {
            return (
                <div className="flex flex-col items-center justify-center p-12 bg-muted/10 border border-dashed border-border rounded-lg">
                    ...
                    <p className="font-medium mb-1">{contentType === 'binary' ? t('content_preview.binary') : t('content_preview.base64')}</p>
            ...
            <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-muted-foreground">
                        {content.length > 1024 ? `${(content.length / 1024).toFixed(2)} KB` : `${content.length} B`}
                    </span>
                </div>

                <div className="flex items-center gap-1">
                    {(contentType === 'json' || contentType === 'html' || contentType === 'xml') && (
                        <Tooltip content={viewMode === 'preview' ? t('content_preview.view_source') : t('content_preview.view_parsed')}>
                            <button
                            ...
                            >
                                {viewMode === 'preview' ? <Eye className="w-4 h-4" /> : <Code className="w-4 h-4" />}
                            </button>
                        </Tooltip>
                    )}
