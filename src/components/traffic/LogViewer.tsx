 ...
 */
const LogLine = memo(({ line, index, colorize }: { line: string; index: number; colorize: (text: string) => React.ReactNode }) => {
    return (
        <div className="whitespace-pre-wrap break-all hover:bg-foreground/5 px-1 rounded flex gap-3 border-l-2 border-transparent hover:border-primary/20">
            <span className="text-muted-foreground/30 select-none w-10 text-right shrink-0 font-mono text-[10px]">{index + 1}</span>
...
export function LogViewer({ onClose }: LogViewerProps) {
    ...
    const [logType, setLogType] = useState<'proxy' | 'app' | 'audit' | 'script' | 'plugin' | 'crash'>('proxy');

    const fetchLogs = async () => {
        setLoading(true);
        try {
            // Fetch last 1000 lines for the active log type
            const data = await invoke<string[]>('get_logs', { logName: logType, lines: 1000 });
            setLogs(data);
        } catch (error) {
    ...
    const colorizeLog = useMemo(() => (text: string) => {
        ...
        html = html.replace(/(\[\d{2}:\d{2}:\d{2}(?:\.\d+)?\])/g, '<span class="text-muted-foreground font-mono text-[11px]">$1</span>');

        // 3. Highlight Keywords
        html = html.replace(/\b(ERROR|FAIL|FATAL|Exception|Panic|CRITICAL)\b/gi, '<span class="text-red-500 font-bold">$1</span>');
        html = html.replace(/\b(CRASH)\b/gi, '<span class="text-red-500 font-bold">$1</span>');
        html = html.replace(/\b(WARN|WARNING)\b/gi, '<span class="text-yellow-500 font-bold">$1</span>');
        ...
        html = html.replace(/\[AUDIT\]/g, `<span class="text-amber-500 font-bold">[${t('log_viewer.audit_logs').replace(/\s*Logs?$/i, '')}]</span>`);
        html = html.replace(/\[SCRIPT\]/g, `<span class="text-purple-500 font-bold">[${t('log_viewer.script_logs').replace(/\s*Logs?$/i, '')}]</span>`);
        html = html.replace(/\[PLUGIN\]/g, `<span class="text-pink-500 font-bold">[${t('log_viewer.plugin_logs').replace(/\s*Logs?$/i, '')}]</span>`);
        html = html.replace(/\[CRASH\]/g, `<span class="text-red-500 font-bold">[${t('log_viewer.crash_logs').replace(/\s*Logs?$/i, '')}]</span>`);

        // 4. Highlight HTTP Methods
        html = html.replace(/\b(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS|CONNECT)\b/g, '<span class="text-purple-400 font-bold">$1</span>');

        // 5. Highlight Status Codes
        html = html.replace(/(\s|\[)([2][0-9]{2})\b/g, '$1<span class="text-green-600 dark:text-green-400">$2</span>');
        html = html.replace(/(\s|\[)([3][0-9]{2})\b/g, '$1<span class="text-blue-600 dark:text-blue-400">$2</span>');
        html = html.replace(/(\s|\[)([45][0-9]{2})\b/g, '$1<span class="text-red-600 dark:text-red-400">$2</span>');
                        ...
                        <div className="flex items-center bg-muted/20 p-1 rounded-xl border border-border/40 gap-0.5 overflow-x-auto no-scrollbar">
                            {[
                                ...
                                <button
                                    ...
                                    className={`px-4 py-1.5 text-[11px] rounded-lg transition-all duration-200 whitespace-nowrap uppercase tracking-wider ${logType === tab.id
                    ...
                    {loading && filteredLogs.length === 0 ? (
                        <div className="absolute inset-0 z-10 flex items-center justify-center bg-background/50 backdrop-blur-sm">
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <span className="text-[10px] uppercase font-black tracking-widest text-muted-foreground/40">{t('common.loading')}</span>
                            </div>
                        </div>
