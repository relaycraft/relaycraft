...
export function BreakpointManager({ variant = 'default' }: BreakpointManagerProps) {
    ...
    const handleClearAll = async () => {
        try {
            ...
            breakpoints.forEach(b => removeBreakpoint(b.id));
        } catch (e) {
        ...
        <div className={containerClass}>
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-black uppercase tracking-widest text-red-500/80">{t('breakpoint.active_manager')}</span>
                </div>
                <button
            ...
            <div className="space-y-1">
                {breakpoints.map((b) => (
                    <div key={b.id} className="group flex items-center justify-between p-2 hover:bg-red-500/10 rounded-xl transition-all border border-transparent hover:border-red-500/20">
