...
import { Copy, Terminal, Workflow, Send, AlertTriangle, Code, RotateCcw } from 'lucide-react';
...
export function useTrafficContextMenu() {
  ...
  const handleToggleBreakpoint = useCallback(async (pattern: string) => {
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent, flow: Flow) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const contextMenuItems: ContextMenuItem[] = menuTargetFlow ? [
    {
      ...
      onClick: () => {
        ...
        notify.success(t('traffic.context_menu.url_copied'), { toastOnly: true });
      }
    },
    {
      ...
      onClick: () => {
        ...
        notify.success(t('traffic.context_menu.curl_copied'), { toastOnly: true });
      }
    },
    { separator: true, label: '' },
    {
      ...
      onClick: () => {
        ...
        if (bodyContent.length > 50000) {
          ...
          notify.warning(t('traffic.context_menu.body_truncated', 'Body truncated for performance. Use "Map Local" for large files.'));
        }
        ...
        setDraftRule({
          ...
          match: {
            request: [
              { type: 'url', value: menuTargetFlow.url, matchType: 'exact' },
              { type: 'method', value: menuTargetFlow.method, matchType: 'equals' }
            ],
            response: []
          },
          actions: [{
            ...
            set: {
              ...
              contentType: menuTargetFlow.contentType || menuTargetFlow.responseHeaders?.['Content-Type'] || menuTargetFlow.responseHeaders?.['content-type']
            }
          }] as any
        });
        setActiveTab('rules');
    ...
    { separator: true, label: '' },
    {
      ...
      onClick: () => {
        if (menuTargetFlow.requestBody) {
          ...
          notify.success(t('traffic.context_menu.req_body_copied'), { toastOnly: true });
        }
      }
    },
    {
      ...
      onClick: () => {
        if (menuTargetFlow.responseBody) {
          ...
          notify.success(t('traffic.context_menu.res_body_copied'), { toastOnly: true });
        }
      }
    },
  ] : [];
