...
import { Sparkles, Loader2, Wand2, X, PlusCircle, Code, Check, SquareCode, FileCode, Bot, RotateCcw } from 'lucide-react';
import { stringifyYAML, parseYAML, validateRuleSchema } from '../../lib/yamlParser';
...
export function AIRuleAssistant({ initialRule, initialMode = 'ai', onApply, onClose, onScriptCreated, setIsDirty }: AIRuleAssistantProps) {
    ...
    }, [initialRule?.id]); // Only trigger sync when rule ID changes to avoid overwriting edits during form typing
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const systemMsg = {
                ...
                content: PROXY_RULE_SYSTEM_PROMPT
            ...
            if (jsonString) {
                try {
                    ...
                    if (aiData.message && !aiData.rule) {
                        ...
                        if (typeof finalMsg === 'string' && finalMsg.startsWith('"') && finalMsg.endsWith('"')) {
                            try {
            ...
            if (detectedIntent === 'rule' && !preview) {
                ...
                setMode('yaml');
                setExplanation(null);
                setYamlErrors([t('rule_editor.ai.generate_fail') + ": " + t('rule_editor.ai.parse_error_hint')]);
            } else if (!preview) {
                setExplanation(fullResponse);
            }
        } catch (error) {
    ...
    const handleApplyYAML = () => {
        try {
            ...
            if (!validation.valid) {
                ...
                const typesStr = VALID_RULE_TYPES.join(', ');
    ...
    const handleCreateScript = async () => {
        ...
        let message = t('rule_editor.convert_script_confirm_dynamic', { name: scriptName });
            ...
            onConfirm: async () => {
                try {
                    ...
                    if (onScriptCreated) {
                        ...
                        setTimeout(() => { useScriptStore.getState().selectScript(scriptName); }, 100);
                        onClose();
                    }
                } catch (error) {
                        ...
                        <>
                            {aiSettings.enabled && (
                                <>
                                            ...
                                            <div className="flex flex-col gap-1 overflow-hidden">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[10px] font-bold text-primary px-1.5 py-0.5 bg-primary/10 rounded uppercase flex-shrink-0">
                                                        {preview.type ? (
                                                            t(`rule_editor.core.types.${preview.type === 'rewrite_header' ? 'rewrite' : preview.type === 'block_request' ? 'block' : preview.type}.label`)
                                                        ) : 'UNKNOWN'}
                                                    </span>
                                                ...
                                                <div className="text-[10px] text-muted-foreground font-mono truncate">
                                                    {t('rule_editor.match.label', 'Match')}: {preview.match?.request?.find(a => a.type === 'url')?.value || ''}
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <Button
                                            ...
                                            <div className="flex items-start gap-4 relative z-10">
                                                ...
                                                <div className="space-y-2 flex-1 overflow-hidden">
                                                    {detectedIntent === 'explain' && (
                                                        <h4 className="text-[10px] font-bold text-primary tracking-widest uppercase">{t('rule_editor.ai.analysis_title')}</h4>
                                                    )}
                                                    <div>
                                                        <AIMarkdown content={explanation || ""} />
                                                        {generating && <span className="inline-block w-1.5 h-4 ml-1 bg-primary animate-pulse align-middle" />}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    {!preview && !generating && !explanation && (
                                        <div />
                                    )}
                                </>
                            )}
                            ...
                            {yamlErrors.length > 0 && (
                                <div className="space-y-1">
                                    {yamlErrors.map((err, i) => (
                                        <div key={i} className="text-[10px] text-destructive flex items-start gap-1 px-1">
                            ...
                            <div className="flex items-center justify-between pt-2">
                                <div className="flex items-center gap-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        />
                                        <label htmlFor="disable-rule" className={`text-xs select-none cursor-pointer ${!initialRule?.id ? 'opacity-50' : ''}`}>
