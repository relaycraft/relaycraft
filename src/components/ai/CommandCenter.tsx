...
import { Sparkles, ArrowRight, CornerDownLeft, Terminal, Settings, Shield, Loader2, Trash2, Power, X, Radar, SendHorizontal, Layers, Braces, Package, FileJson, MessageSquare, Lock, AlertTriangle } from 'lucide-react';
...
export function CommandCenter() {
    ...
    const { isOpen, setIsOpen, input, setInput, addHistory, suggestions, setSuggestions } = useCommandStore();
    ...
    const { startProxy, stopProxy, running } = useProxyStore();
    ...
    }, [isOpen, aiSettings.enabled]);

    useEffect(() => {
        if (isOpen) {
            updateContextualSuggestions();
        }
    }, [isOpen, input, activeTab, selectedFlow, aiSettings.enabled, running]);
    ...
    const handleRunCommand = async (forceInput?: string) => {
        ...
        try {
            ...
            if (result.intent !== 'CHAT' && !(result.intent === 'CREATE_SCRIPT' && result.params?.requirement)) {
                setStreamingMessage(null);
            }

            const SAFE_INTENTS = ['NAVIGATE', 'OPEN_SETTINGS', 'CREATE_RULE', 'CREATE_SCRIPT', 'CLEAR_TRAFFIC', 'GENERATE_REQUEST', 'CHAT'];
            ...
            } else if (SAFE_INTENTS.includes(result.intent)) {
                await executeAction(result);
            } else {
                setAction(result);
            }
            addHistory(commandToRun);
            SuggestionEngine.recordUsage(commandToRun.startsWith('/') ? commandToRun.split(' ')[0] : commandToRun);
        } catch (error) {
    ...
    const executeAction = async (forcedAction?: CommandAction) => {
        ...
        switch (act.intent) {
            ...
            case 'CREATE_RULE':
                selectRule(null);
                const rawRule = act.params?.rule || { name: act.params?.description || 'AI Rule' };
            ...
            case 'CREATE_SCRIPT': {
                ...
                const requirement = act.params?.requirement || act.params?.description || act.params?.message || "";
                                ...
                                <input
                                    ...
                                    placeholder={
                                        isEditingRule
                                            ...
                                            : (aiSettings.enabled ? t('command_center.placeholder.ai') : t('command_center.placeholder.default'))
                                    }
                                    ...
                                    <div className="px-2 py-1.5 flex items-center justify-between min-h-[32px]">
                                        <div className="flex flex-wrap gap-1 items-center">
                                            ...
                                            <span className="text-[9px] bg-muted/50 text-muted-foreground/70 h-5 px-1.5 rounded-md border border-border/40 flex items-center gap-1 font-bold">
                                                {(() => {
                                                    const page = pluginPages.find(p => p.id === activeTab || p.route === activeTab);
                                                    if (page?.icon) {
                                                    ...
                                                    if (activeTab === 'composer') return <SendHorizontal className="w-3 h-3" />;
                                                ...
                                                {(() => {
                                                    if (['traffic', 'composer', 'rules', 'scripts', 'plugins', 'certificate', 'settings'].includes(activeTab)) {
                                            ...
                                            {/* Flow Context */}
                                            {selectedFlow && (
                                                <span className="text-[9px] bg-muted/50 text-muted-foreground/70 h-5 px-1.5 rounded-md border border-border/40 flex items-center gap-1 font-bold animate-in fade-in zoom-in-95" title={selectedFlow.url}>
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            {aiSettings.enabled && (
                                                <div className="flex items-center gap-1 bg-muted/30 px-1.5 py-0.5 rounded-md border border-white/5" title="AI Active">
                                    ...
                                    {suggestions.map((s, i) => (
                                        ...
                                        >
                                            <div className="p-1.5 bg-muted rounded-md group-hover:bg-primary/10 transition-colors">
                                                {s.group === 'navigation' && <CornerDownLeft className="w-3.5 h-3.5 text-muted-foreground group-hover:text-primary" />}
                                                {s.group === 'action' && <Terminal className="w-3.5 h-3.5 text-muted-foreground group-hover:text-primary" />}
                                                {s.group === 'ai' && <Sparkles className="w-3.5 h-3.5 text-muted-foreground group-hover:text-primary" />}
                                            </div>
                                            <div className="flex-1 text-left">
                                                <div className="font-medium text-foreground/80 group-hover:text-foreground">{s.label}</div>
                                                {s.description && (
                                                    <div className="text-[10px] text-muted-foreground/50 group-hover:text-muted-foreground/70">{s.description}</div>
                                                )}
                                            </div>
                                        </button>
                                    ))}
                                        ...
                                        <div className="space-y-2">
                                            <div className="p-3 bg-muted/30 border border-border/50 rounded-xl relative overflow-hidden animate-in fade-in slide-in-from-bottom-1 duration-300">
                                                <div className="text-system leading-relaxed text-foreground/90 font-medium px-1 prose-compact">
                                                    <AIMarkdown content={streamingMessage.replace(/```(?:python)?\s*$/i, '')} />
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {executing && !streamingMessage && !error && (
                                        <div className="py-8 flex flex-col items-center justify-center gap-3 animate-in fade-in duration-300">
                                            ...
                                            <div className="text-xs font-bold text-muted-foreground/60 animate-pulse tracking-widest uppercase">
                                                {aiSettings.enabled ? t('command_center.loading.ai') : t('command_center.loading.default')}
                                            </div>
                                        </div>
                                    )}
                                </div>
                                        ...
                                        <div className="relative p-5 space-y-3">
                                            ...
                                            <div className="flex items-start justify-between">
                                                <div className="flex items-center gap-3">
                                                    <div className={`p-2.5 rounded-xl border border-white/5 shadow-inner ${action.intent === 'CLEAR_TRAFFIC'
                                                        ? 'bg-destructive/10 text-destructive'
                                                        : (action.intent === 'TOGGLE_PROXY' ? 'bg-orange-500/10 text-orange-500' : 'bg-primary/10 text-primary')
                                                        }`}>
