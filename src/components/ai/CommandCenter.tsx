...
export function CommandCenter() {
  ...
  const handleRunCommand = async (forceInput?: string) => {
    ...
    try {
      ...
      // For CHAT and CREATE_SCRIPT/CREATE_RULE with requirements, keep streamingMessage visible
      if (
        result.intent !== "CHAT" &&
        !(result.intent === "CREATE_SCRIPT" && result.params?.requirement) &&
        !(result.intent === "CREATE_RULE" && result.params?.requirement)
      ) {
        setStreamingMessage(null);
      }
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-2xl bg-popover/98 backdrop-blur-xl border border-border/40 rounded-xl shadow-2xl overflow-hidden flex flex-col ring-1 ring-border/40 text-left"
