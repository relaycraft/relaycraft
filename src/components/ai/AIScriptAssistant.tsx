...
export function AIScriptAssistant({
    ...
    async (mode: "generate" | "explain" = "generate", overridePrompt?: string) => {
      ...
      try {
        ...
        if (mode === "explain") {
          ...
          userMsg = {
            role: "user",
            content: getScriptGenerationPrompt(activePrompt, currentCode),
          };
        }
  ...
  useEffect(() => {
    if (hasHandledPromptRef.current) return;

    // Read the current state directly to avoid dependency triggers
    const currentDraftPrompt = useUIStore.getState().draftScriptPrompt;
    const targetPrompt = initialPrompt || currentDraftPrompt;
    ...
    if (targetPrompt) {
      ...
      if (targetPrompt === "INITIAL_OPEN_ONLY") {
        ...
        handleGenerate("generate", targetPrompt);
      }
      if (currentDraftPrompt) setDraftScriptPrompt(null);
    }
  }, [initialPrompt, setDraftScriptPrompt, handleGenerate]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {((explanation && explanation.trim().length > 0) || generating) &&
          genMode !== "generate" && (
            <div className="flex gap-4 p-4 bg-background/50 border border-border/50 rounded-2xl relative overflow-hidden">
              ...
              <div className="flex-1 space-y-2 min-w-0">
                ...
                <div
                  className="text-xs text-foreground/80 leading-[1.6] max-h-[220px] overflow-y-auto pr-2 no-scrollbar scroll-smooth font-medium"
                  ref={scrollRef}
                >
                  <AIMarkdown content={cleanInsight(explanation)} />
                  {generating && (
                    <motion.span
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
