...
import { Loader2, Bot, Wand2, X, Check } from 'lucide-react';
import { motion } from 'framer-motion';
import { useAIStore } from '../../stores/aiStore';
import { useUIStore } from '../../stores/uiStore';
import { MITMPROXY_SYSTEM_PROMPT, SCRIPT_EXPLAIN_SYSTEM_PROMPT, getScriptGenerationPrompt, getScriptExplanationPrompt } from '../../lib/ai/prompts';
...
export function AIScriptAssistant({ onApply, onClose, currentCode, isCreateMode, initialPrompt }: AIScriptAssistantProps) {
    ...
    }, [draftScriptPrompt, initialPrompt]);
    ...
    const handleGenerate = async (mode: 'generate' | 'explain' = 'generate') => {
        ...
        try {
            ...
            if (mode === 'explain') {
                ...
                systemMsg = {
                    role: 'system' as const,
                    content: MITMPROXY_SYSTEM_PROMPT
                        .replace(/{{LANGUAGE}}/g, langInfo.name)
                        .replace(/{{TERMINOLOGY}}/g, 'RelayCraft, mitmproxy, Addon')
                };
                userMsg = { role: 'user' as const, content: getScriptGenerationPrompt(prompt, currentCode) };
            }
            ...
            <div className="flex flex-col w-full p-4 gap-4">
                ...
                <div className="flex items-center gap-2 px-3.5 bg-muted/30 border border-border/20 shadow-sm rounded-xl transition-all focus-within:ring-2 focus-within:ring-primary/40 focus-within:border-primary/40">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={onClose} title={t('common.close')} className="p-1.5 hover:bg-muted rounded-lg text-muted-foreground transition-all">
                ...
                {(explanation || generating) && !(genMode === 'generate' && tempCode && !explanation) && (
                    <div className="flex gap-4 p-4 bg-background/50 border border-border/50 rounded-2xl relative overflow-hidden">
                        ...
                        <div className="flex-1 space-y-2 min-w-0">
                                    ...
                                    {tempCode && (
                                        <div className="flex items-center gap-2">
                                            <motion.div
                                                ...
                                                title={generating ? t('script_editor.ai.syncing') : t('script_editor.ai.synced')}
                                            >
                            ...
                            <div className="text-[12px] text-foreground/80 leading-[1.6] max-h-[220px] overflow-y-auto pr-2 no-scrollbar scroll-smooth font-medium" ref={scrollRef}>
                                <AIMarkdown content={genMode === 'generate' && generating ? "" : cleanInsight(explanation)} />
                                {generating && (
                                    <motion.span
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
                ...
                {!generating && !explanation && (
                    <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
                        {isCreateMode ? (
                            <>
                                <QuickAction label={t('script_editor.ai.chip_create')} onClick={() => setPrompt(t('script_editor.ai.prompt_create'))} />
                                <QuickAction label={t('script_editor.ai.chip_custom_logic', 'Generate specific logic...')} onClick={() => setPrompt(t('script_editor.ai.prompt_custom_logic', 'Help me write a logic that: '))} />
                            </>
                        ) : (
                            <>
                                <QuickAction label={t('script_editor.ai.chip_explain')} onClick={() => handleGenerate('explain')} />
                                <QuickAction label={t('script_editor.ai.chip_modify')} onClick={() => setPrompt(t('script_editor.ai.prompt_modify'))} />
                                <QuickAction label={t('script_editor.ai.chip_fix')} onClick={() => setPrompt(t('script_editor.ai.prompt_fix'))} />
                            </>
                        )}
                    </div>
                )}
