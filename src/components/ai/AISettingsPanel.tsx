...
import { Loader2, CheckCircle, XCircle, Eye, EyeOff, Save, RotateCw } from 'lucide-react';
import { useAIStore } from '../../stores/aiStore';
import { useTranslation } from 'react-i18next';
import { SettingsSection, SettingsRow, SettingsInput, SettingsSelect, SettingsToggle } from '../settings/SettingsLayout';
...
export function AISettingsPanel() {
    ...
    const handleSave = async () => {
        try {
            ...
            alert(t('ai.connection.save') + ' Failed: ' + (error instanceof Error ? error.message : JSON.stringify(error)));
        }
    };
        ...
        <SettingsSection title={t('ai.title')}>
            ...
            {localSettings.enabled && (
                <>
                    ...
                    <SettingsRow title={t('ai.provider')} description={t('ai.provider_desc')}>
                        <SettingsSelect
                            ...
                            onChange={(val) => {
                                ...
                                useAIStore.getState().getProviderKey(val).then((key) => {
                                    setLocalSettings({
                                        ...localSettings,
                                        provider: val as any,
                    ...
                    {(localSettings.provider === 'custom' || (localSettings.customEndpoint && localSettings.customEndpoint !== getProviderById(localSettings.provider)?.defaultEndpoint)) && (
                        <SettingsRow
                            title={t('ai.endpoint')}
                            description={t('ai.endpoint_desc')}
                        >
                            <SettingsInput
                    ...
                    <SettingsRow title={t('ai.api_key')} description={t('ai.api_key_desc')}>
                        <div className="relative flex items-center gap-2">
                            <SettingsInput
                                ...
                                placeholder={localSettings.provider === 'openai' ? t('ai.connection.placeholder_openai') : t('ai.connection.placeholder_optional')}
                            />
                            <button
                    ...
                    <SettingsRow title={t('ai.memory_turns')} description={t('ai.memory_turns_desc')}>
                        <SettingsInput
                            ...
                            onChange={(e) => setLocalSettings({ ...localSettings, maxHistoryMessages: parseInt(e.target.value) })}
                        />
                    </SettingsRow>
                    ...
                    <div className="flex items-center justify-between p-3 bg-muted/10 border-t border-border/40">
                        <div className="flex items-center gap-2">
                            ...
                            {connectionStatus !== 'idle' && (
                                <div className={`flex items-center gap-1.5 px-2 py-1 rounded text-[11px] font-medium ${connectionStatus === 'success'
                                    ? 'text-green-600 bg-green-500/10'
                                    : 'text-destructive bg-destructive/10'
                                    }`}>
                                    {connectionStatus === 'success' ? <CheckCircle className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
                                    {connectionStatus === 'success' ? t('ai.connection.success') : t('ai.connection.failure')}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                disabled={testingConnection || (localSettings.provider !== 'custom' && !localSettings.apiKey)}
                            ...
                            >
                                {testingConnection ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RotateCw className="w-3.5 h-3.5" />}
