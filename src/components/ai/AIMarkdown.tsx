...
export function AIMarkdown({ content, className = "" }: AIMarkdownProps) {
    ...
    return (
        <div className={`text-system leading-relaxed text-foreground/90 font-medium prose-compact ${className}`}>
            <AnimatePresence>
                {thoughts && (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <div className="p-1.5 bg-primary/10 rounded-lg group-hover:bg-primary/20 transition-colors">
                                    <Brain className={`w-3.5 h-3.5 text-primary/60 group-hover:text-primary ${isThinking ? 'animate-pulse' : ''}`} />
                                </div>
                                {isThinking && (
                            ...
                            <div className="flex items-center gap-2 pr-1">
                                <ChevronDown className={`w-3.5 h-3.5 text-primary/30 group-hover:text-primary/60 transition-transform duration-300 ${isCollapsed ? '-rotate-90' : ''}`} />
                            </div>
                        </button>
            ...
            {answer && (
                <ReactMarkdown
                    ...
                    components={{
                        h1: ({ children }: any) => <h1 className="text-base font-bold text-foreground mb-2 mt-2 flex items-center gap-2 border-b border-border/40 pb-1.5">{children}</h1>,
                        h2: ({ children }: any) => <h2 className="text-sm font-bold text-foreground mb-2 mt-4 flex items-center gap-2">{children}</h2>,
                        h3: ({ children }: any) => <h3 className="text-system font-bold text-foreground/90 mb-1.5 mt-3">{children}</h3>,
                        p: ({ children }: any) => <p className="mb-2 last:mb-0 leading-relaxed overflow-hidden text-foreground/80">{children}</p>,
                        code: ({ children }: any) => <code className="bg-primary/10 px-1.5 py-0.5 rounded text-[11px] font-mono border border-primary/20 text-primary font-medium">{children}</code>,
                        pre: ({ children }: any) => <pre className="bg-muted/50 p-4 rounded-xl border border-border/40 my-3 overflow-x-auto no-scrollbar font-mono text-[11px] leading-relaxed select-all shadow-inner">{children}</pre>,
                        ul: ({ children }: any) => <ul className="list-disc ml-4 space-y-2 mb-4 text-foreground/80">{children}</ul>,
                        ol: ({ children }: any) => <ol className="list-decimal ml-4 space-y-2 mb-4 text-foreground/80">{children}</ol>,
                        ...
                        th: ({ children }: any) => <th className="border-b border-border/40 px-3 py-2 text-left font-bold text-foreground/70 uppercase tracking-wider bg-white/[0.03] whitespace-nowrap">{children}</th>,
                        td: ({ children }: any) => <td className="border-b border-border/20 px-3 py-2 text-foreground/80">{children}</td>,
                        tr: ({ children }: any) => <tr className="hover:bg-white/[0.01] transition-colors last:child:border-0">{children}</tr>
                    }}
                >
                    {answer}
                </ReactMarkdown>
