...
const markdownComponents: any = {
  ...
  code: ({ node, inline, className, children, ...props }: any) => {
    ...
    if (isInline) {
      return (
        <code
          className="bg-primary/10 px-1.5 py-0.5 rounded text-ui font-mono border border-primary/20 text-primary font-medium"
          {...props}
        >
          {children}
        </code>
    ...
    return (
      <div className="relative group my-4 overflow-hidden rounded-xl border border-border/40 shadow-inner">
        <div className="flex items-center justify-between px-3 py-1.5 bg-muted/80 border-b border-border/40">
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-medium text-muted-foreground uppercase opacity-80">
              {language}
            </span>
          </div>
          <CopyButton
            text={textContent}
            variant="quiet"
            className="opacity-0 group-hover:opacity-100 transition-opacity border-transparent bg-transparent text-muted-foreground hover:text-foreground hover:bg-muted/50"
          />
        </div>
        <pre className="bg-muted/30 p-4 overflow-x-auto no-scrollbar font-mono text-ui leading-relaxed shadow-inner m-0">
          <code className="text-foreground/90 font-medium" {...props}>
            {children}
          </code>
        </pre>
      </div>
  ...
  tr: ({ children }: any) => (
    <tr className="hover:bg-white/[0.01] transition-colors last:child:border-0">{children}</tr>
  ),
};
...
export function AIMarkdown({ content, className = "" }: AIMarkdownProps) {
  ...
  return (
    ...
    >
      ...
      {answer && (
        <div className="relative group/markdown pt-0.5">
          <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>
            {answer}
          </ReactMarkdown>

          <div className="absolute top-0 right-0 z-10 opacity-0 pointer-events-none group-hover/markdown:opacity-100 group-hover/markdown:pointer-events-auto transition-opacity duration-200">
            <CopyButton
              text={answer}
              variant="secondary"
              tooltipSide="left"
