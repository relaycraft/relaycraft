...
import { Sparkles, Loader2, X, BookOpen, ArrowLeft, Bot, Wand2 } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useAIStore } from '../../stores/aiStore';
import { useUIStore } from '../../stores/uiStore';
import { Tooltip } from '../common/Tooltip';
import { FILTER_ASSISTANT_SYSTEM_PROMPT, REGEX_ASSISTANT_SYSTEM_PROMPT, NAMING_ASSISTANT_SYSTEM_PROMPT, REGEX_EXPLAIN_SYSTEM_PROMPT } from '../../lib/ai/prompts';
...
export function AIAssistant({ onGenerate, className = "", mode, context, value, align = 'right' }: AIAssistantProps) {
    ...
    const handleExplain = async () => {
        ...
        try {
            ...
            const systemMsg = {
                role: 'system' as const,
                content: REGEX_EXPLAIN_SYSTEM_PROMPT
                    .replace(/{{LANGUAGE}}/g, langInfo.name)
                    .replace(/{{TERMINOLOGY}}/g, langInfo.terminology)
                    .replace(/{{ACTIVE_TAB}}/g, useUIStore.getState().activeTab)
            ...
            const userMsg = { role: 'user' as const, content: `Please explain this content: ${value}` };
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-[10px] font-bold text-primary uppercase tracking-widest">
                                            {view === 'input' ? config.title : (loading ? t('ai_assistant.analyzing') : config.title)}
                                        </span>
                                    </div>
                                </div>
                                <button onClick={() => setIsOpen(false)} className="p-1.5 hover:bg-white/5 dark:hover:bg-white/10 rounded-full transition-colors">
