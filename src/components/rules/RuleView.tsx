...
import { Plus, Folders, AlertTriangle, X } from 'lucide-react';
import { getRuleConflicts } from '../../stores/ruleStore';
import { useTranslation } from 'react-i18next';

export function RuleView() {
    const { t } = useTranslation();
    const {
        rules, groups, addGroup, selectedRule, selectRule,
        loadRules, draftRule, setDraftRule, searchQuery,
        isEditorDirty, setIsEditorDirty, ruleGroups,
        loadErrors, clearLoadErrors
    } = useRuleStore();
    ...
    const groupedRules = filteredRules.reduce((acc: Record<string, Rule[]>, rule: Rule) => {
        ...
        <div className="h-full flex overflow-hidden">
                ...
                >
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 space-y-2">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-sm font-bold text-destructive">{t('rules.alerts.load_error_title', 'Corrupted Rules Detected')}</h3>
                                        <button
                                    ...
                                    <p className="text-xs text-muted-foreground leading-relaxed">
                                        {t('rules.alerts.load_error_msg', 'Some rule files on disk could not be parsed. They have been skipped but you may want to fix them manually.')}
                                    </p>
                                    <div className="space-y-1 mt-3">
                                        {loadErrors.slice(0, 3).map((err, idx) => (
                                            <div key={idx} className="flex flex-col gap-0.5 p-2 rounded-lg bg-background/50 border border-border/10">
                                                <div className="text-[10px] font-mono text-foreground truncate">{err.path.split(/[\\/]/).pop()}</div>
                                                <div className="text-[10px] text-destructive/80 italic line-clamp-2">{err.error}</div>
                                            </div>
                                        ))}
                                        {loadErrors.length > 3 && (
                    ...
                    {filteredRules.length === 0 && groups.length === 0 ? (
                        ...
                        <div className="pb-20 space-y-4">
                            ...
                            {/* Groups with rules */}
                            {[...groups].sort((a: RuleGroup, b: RuleGroup) => {
                            ...
                            {groupedRules['uncategorized'] && groupedRules['uncategorized'].length > 0 && (
                                <div className="border-b border-border/20">
                                    <div className="flex items-center px-4 py-2 bg-muted/40 border-y border-border/40 select-none">
                                        ...
                                        <span className="text-system font-bold tracking-tight text-muted-foreground">{t('rules.uncategorized')}</span>
            ...
            <AnimatePresence>
                {showEditor && (
                    ...
                    >
                        <RuleEditor
                            key={selectedRule?.id || (draftRule as any)?._draftId || (draftRule ? 'draft' : 'none')}
