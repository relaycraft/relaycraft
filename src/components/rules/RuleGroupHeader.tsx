import { ChevronDown, ChevronRight, Edit, Trash2, Library, ChevronUp, ChevronDown as ChevronDownIcon } from 'lucide-react';
import { RuleGroup } from '../../types/rules';
...
export function RuleGroupHeader({ group, ruleCount }: RuleGroupHeaderProps) {
    ...
    const {
        toggleGroup, toggleGroupCollapse, deleteGroup, updateGroup,
        moveGroup, groups
    } = useRuleStore();
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0 flex items-center gap-2">
                <Library className={`w-3.5 h-3.5 ${group.enabled ? 'text-primary' : 'text-muted-foreground/50'}`} />
                ...
                {isEditing ? (
                    ...
                    <div className="flex items-center gap-2 truncate">
                        <span className={`text-system font-bold tracking-tight ${group.enabled ? 'text-foreground' : 'text-muted-foreground'}`}>
                            {group.name}
                        </span>
            ...
            <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center gap-1 opacity-0 group-hover/group-header:opacity-100 transition-all duration-200">
                    ...
                    <Tooltip content={t('rule_group.move_down')}>
                        <button
                            ...
                            disabled={groups.sort((a, b) => a.priority - b.priority)[groups.length - 1]?.id === group.id}
                            className="p-1 hover:bg-muted rounded text-muted-foreground hover:text-primary disabled:opacity-20 disabled:cursor-not-allowed"
                        >
                ...
                <Tooltip content={group.enabled ? t('rule_group.disable_group') : t('rule_group.enable_group')}>
                    <Switch
                        size="sm"
