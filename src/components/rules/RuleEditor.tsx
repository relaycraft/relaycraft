...
import { X, Save, Settings2, Plus, Bot } from 'lucide-react';
            ...
            <div className="flex items-center justify-between px-6 py-2.5 border-b border-border bg-background/80 backdrop-blur-md sticky top-0 z-20">
                <div className="flex items-center gap-3">
                    <div className="p-1.5 bg-primary/10 rounded-lg">
                        {rule?.id ? <Settings2 className="w-4 h-4 text-primary" /> : <Plus className="w-4 h-4 text-primary" />}
                    </div>
                ...
                {ruleType !== 'block_request' && (
                    <section className="space-y-4">
                        ...
                        {ruleType === 'rewrite_body' && (
                            <ActionRewrite
                                target={rewriteTarget} onChangeTarget={setRewriteTarget}
                                type={rewriteType} onChangeType={setRewriteType}
                                content={rewriteContent} onChangeContent={setRewriteContent}
                                statusCode={rewriteStatusCode} onChangeStatusCode={setRewriteStatusCode}
                                contentType={rewriteContentType} onChangeContentType={setRewriteContentType}
                                pattern={rewritePattern} onChangePattern={setRewritePattern}
                                replacement={rewriteReplacement} onChangeReplacement={setRewriteReplacement}
                                jsonModifications={jsonModifications} onChangeJsonModifications={setJsonModifications}
                            />
                        )}
                        ...
                        {ruleType === 'map_local' && (
                            <>
                                <ActionMapLocal
                                    source={mapLocalSource} onChangeSource={setMapLocalSource}
                                    localPath={localPath} onChangeLocalPath={setLocalPath}
                                    content={mapLocalContent} onChangeContent={setMapLocalContent}
                                    statusCode={mapLocalStatusCode} onChangeStatusCode={setMapLocalStatusCode}
                                    contentType={contentType} onChangeContentType={setContentType}
                                />
                                <div className="mt-4">
                                    <ActionHeader
                                        headersRequest={headersRequest} onChangeHeadersRequest={setHeadersRequest}
                                        headersResponse={headersResponse} onChangeHeadersResponse={setHeadersResponse}
                                    />
                                </div>
                            </>
                        )}
                        ...
                        {ruleType === 'map_remote' && (
                            <ActionMapRemote
                                targetUrl={targetUrl} onChangeTargetUrl={setTargetUrl}
                                preservePath={preservePath} onChangePreservePath={setPreservePath}
                                headersRequest={headersRequest} onChangeHeadersRequest={setHeadersRequest}
                                headersResponse={headersResponse} onChangeHeadersResponse={setHeadersResponse}
                            />
                        )}
                        ...
                        {ruleType === 'throttle' && (
                            <ActionThrottle
                                delayMs={delayMs} onChangeDelayMs={setDelayMs}
                                packetLoss={packetLoss} onChangePacketLoss={setPacketLoss}
                                bandwidthKbps={bandwidthKbps} onChangeBandwidthKbps={setBandwidthKbps}
                            />
                        )}
