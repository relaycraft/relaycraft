...
import { Rule, RuleType, UrlMatchType, HttpMethod, RuleAction, HeaderOperation, JsonModification } from '../../../types/rules';
...
export function useRuleEditor({ rule, onClose }: UseRuleEditorProps) {
    ...
    const [requiredHeaders, setRequiredHeaders] = useState<Array<{ key: string; value?: string; matchType: 'contains' | 'exact' | 'regex' }>>(
        (rule?.match?.request?.filter(a => a.type === 'header')?.map(a => ({
            key: a.key!,
    ...
    const [rewriteType, setRewriteType] = useState<'replace' | 'regex_replace' | 'set' | 'json'>('set');
    ...
    const getCurrentRuleObject = useCallback((): Partial<Rule> => {
        const currentRule: Partial<Rule> = {
            ...
            match: {
                request: [
                    ...
                    ...(methods.length > 0 ? [{ type: 'method', matchType: 'exact', value: methods } as any] : []),
        ...
        if (ruleType === 'rewrite_body') {
            ...
            if (rewriteType === 'set') {
                ...
                action.replace = { pattern: rewritePattern, replacement: rewriteReplacement };
            } else if (rewriteType === 'regex_replace') {
                action.regex_replace = { pattern: rewritePattern, replacement: rewriteReplacement };
            ...
            currentRule.actions!.push({
                ...
                headers: (headersRequest.length > 0 || headersResponse.length > 0) ? {
        ...
        rule, name, ruleType, urlPattern, urlMatchType, methods, requiredHeaders,
        rewriteTarget, rewriteType, rewriteContent, rewritePattern, rewriteReplacement, jsonModifications, rewriteStatusCode, rewriteContentType,
        mapLocalSource, localPath, mapLocalContent, contentType, mapLocalStatusCode, headersRequest, headersResponse,
        targetUrl, preservePath, delayMs, packetLoss, bandwidthKbps
    ]);
    ...
    useEffect(() => {
        ...
        if (rule.match) {
            ...
            setRequiredHeaders(headerAtoms.map(a => ({
                key: a.key!,
                matchType: (a.matchType as any) || 'exact',
                value: a.value as string
            })));
        ...
        actions.forEach(a => {
            if (a.type === 'rewrite_body') {
                ...
                if (a.statusCode !== undefined) setRewriteStatusCode(a.statusCode === null ? undefined : a.statusCode);
                if (a.contentType !== undefined) setRewriteContentType(a.contentType === null ? undefined : a.contentType);
                ...
                if (a.headers) {
                    if (a.headers.request && a.headers.request.length > 0) setHeadersRequest(a.headers.request);
                    if (a.headers.response && a.headers.response.length > 0) setHeadersResponse(a.headers.response);
                }
    ...
    const handleApplyAIRule = useCallback((partialRule: Partial<Rule>) => {
        ...
        if (partialRule.match) {
            ...
            if (atoms.length > 0) {
                ...
                const urlAtom = atoms.find(a => a.type === 'url' || a.type === 'path' || a.type === 'host');
                if (urlAtom) {
                    setUrlPattern(urlAtom.value as string || '');
                    setUrlMatchType((urlAtom.matchType as UrlMatchType) || 'contains');
                }
        ...
        setName, setRuleType, setUrlPattern, setUrlMatchType, setMethods, setRequiredHeaders,
        setRewriteTarget, setRewriteStatusCode, setRewriteContentType, setRewriteType, setRewriteContent,
        setRewritePattern, setRewriteReplacement, setJsonModifications,
        setMapLocalSource, setLocalPath, setMapLocalContent, setContentType, setMapLocalStatusCode,
        setHeadersRequest, setHeadersResponse, setTargetUrl, setPreservePath,
        setDelayMs, setPacketLoss, setBandwidthKbps
    ]);
    ...
    const handleSave = useCallback(() => {
        ...
        if (ruleType === 'rewrite_body') {
            ...
            actions.push({
                ...
                contentType: contentType || undefined,
                statusCode: mapLocalStatusCode,
                headers: (headersResponse.length > 0) ? {
                    request: [],
                    response: headersResponse
                } : undefined
            });
        ...
        if (!finalName) {
            ...
            if (ruleType === 'map_local' || ruleType === 'map_remote' || ruleType === 'throttle' || ruleType === 'rewrite_body') labelKey = ruleType;

            const rawLabel = t(`rule_editor.core.types.${labelKey}.label`);
            const prettyUrl = urlPattern ? (urlPattern.length > 30 ? urlPattern.substring(0, 28) + '...' : urlPattern) : 'Global';
        ...
        rule, ruleType, rewriteTarget, rewriteStatusCode, rewriteContentType, rewriteType, rewriteContent,
        rewritePattern, rewriteReplacement, jsonModifications, jsonModifications,
        headersRequest, mapLocalSource, localPath, mapLocalContent, contentType, mapLocalStatusCode, headersResponse,
        targetUrl, preservePath, delayMs, packetLoss, bandwidthKbps, name, urlPattern, urlMatchType, methods, requiredHeaders,
        t, groupId, updateRule, addRule, setIsEditorDirty, onClose
    ]);
    ...
    const checkDirty = useCallback(() => {
        ...
        if (!rule?.id) {
            ...
            if (ruleType === 'map_local' && (localPath.trim() !== '' || mapLocalContent.trim() !== '')) return true;
            if (ruleType === 'rewrite_body' && (rewriteContent.trim() !== '' || rewritePattern.trim() !== '')) return true;
            if (ruleType === 'throttle' && (delayMs > 0 || packetLoss > 0 || bandwidthKbps > 0)) return true;
            if (ruleType === 'rewrite_header' && (headersRequest.length > 0 || headersResponse.length > 0)) return true;
        }
        ...
        const currentUrlPattern = initialRule?.match?.request?.find(a => a.type === 'url')?.value || '';
        const currentUrlMatchType = initialRule?.match?.request?.find(a => a.type === 'url')?.matchType || 'contains';
        if (urlPattern !== currentUrlPattern) return true;
        if (urlMatchType !== currentUrlMatchType) return true;

        const initialMethods = initialRule?.match?.request?.find(a => a.type === 'method')?.value || [];
        if (JSON.stringify([...methods].sort()) !== JSON.stringify([...(initialMethods as string[])].sort())) return true;

        const initialRequiredHeaders = initialRule?.match?.request?.filter(a => a.type === 'header')?.map(a => ({
            key: a.key!,
            matchType: (a.matchType as any) || 'exact',
            value: a.value as string
        })) || [];
        ...
        if (ruleType === 'rewrite_body') {
            ...
            const normalizedInitialStatusCode = initialStatusCode === null ? undefined : initialStatusCode;
            ...
            const normalizedInitialContentType = initialContentType === null ? undefined : initialContentType;
            ...
            if (rewriteType === 'set') {
                ...
                if (JSON.stringify(jsonModifications) !== JSON.stringify(initialAction.json.modifications || [])) return true;
            }
        } else if (ruleType === 'map_local') {
            ...
            if (preservePath !== (initialAction?.preservePath || false)) return true;
            if (JSON.stringify(headersRequest) !== JSON.stringify(initialHeadersReq)) return true;
            if (JSON.stringify(headersResponse) !== JSON.stringify(initialHeadersRes)) return true;
        } else if (ruleType === 'throttle') {
        ...
        name, ruleType, groupId, urlPattern, urlMatchType, methods, requiredHeaders,
        rewriteTarget, rewriteType, rewriteContent, rewritePattern, rewriteReplacement, jsonModifications, rewriteStatusCode, rewriteContentType,
        mapLocalSource, localPath, mapLocalContent, contentType, mapLocalStatusCode, headersRequest, headersResponse,
        targetUrl, preservePath, delayMs, packetLoss, bandwidthKbps,
        rule, initialRule, ruleGroups
    ]);
    ...
    return {
        ...
        urlMatchType, setUrlMatchType,
        methods, setMethods,
        requiredHeaders, setRequiredHeaders,
        rewriteTarget, setRewriteTarget,
        rewriteType, setRewriteType,
        rewriteContent, setRewriteContent,
        rewritePattern, setRewritePattern,
        rewriteReplacement, setRewriteReplacement,
        jsonModifications, setJsonModifications,
        rewriteStatusCode, setRewriteStatusCode,
        rewriteContentType, setRewriteContentType,
        headersRequest, setHeadersRequest,
        headersResponse, setHeadersResponse,
        mapLocalSource, setMapLocalSource,
        localPath, setLocalPath,
        mapLocalContent, setMapLocalContent,
        contentType, setContentType,
        mapLocalStatusCode, setMapLocalStatusCode,
        targetUrl, setTargetUrl,
        preservePath, setPreservePath,
        delayMs, setDelayMs,
        packetLoss, setPacketLoss,
        bandwidthKbps, setBandwidthKbps,
