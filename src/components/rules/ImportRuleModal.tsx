...
export function ImportRuleModal() {
    ...
    const handleImport = () => {
        try {
            ...
            notify.error('Invalid rule format. Please use a valid Bundle or Single Rule JSON.', t('common.error'));
            setImportModalOpen(false);
            setJsonText('');
        } catch (e: any) {
            ...
            if (path) {
                if (path.toLowerCase().endsWith('.zip')) {
                    ...
                    try {
                        ...
                        if (result.success) {
                            notify.success(t('rules.import_zip_success', { count: result.imported }), t('sidebar.rules'));
                            setImportModalOpen(false);
                        } else {
