    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
                <span className="text-[11px] font-bold text-foreground/50 uppercase tracking-widest">{label || t('header_editor.title')}</span>
            </div>
            <div className="space-y-2">
                {operations.map((op, index) => (
                    <div key={index} className="grid grid-cols-[1.5fr_130px_1fr_auto] gap-2 items-center group">
                        <Input
                            type="text"
                            value={op.key}
                            onChange={(e) => updateOperation(index, 'key', e.target.value)}
                            placeholder="Header Key"
                            className="font-mono text-system placeholder:text-xs w-full bg-muted/20 border-border/40 focus:bg-background transition-colors"
                        />

                        <Select
                            value={op.operation}
                            onChange={(val) => updateOperation(index, 'operation', val)}
                            className="h-[34px] py-1 text-system w-full bg-muted/20 border-border/40"
