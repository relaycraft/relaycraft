...
import { JsonModification } from '../../../../types/rules';
...
export function ActionRewrite({
    ...
    statusCode, onChangeStatusCode,
    contentType, onChangeContentType,
    pattern, onChangePattern,
    replacement, onChangeReplacement,
    jsonModifications, onChangeJsonModifications
}: ActionRewriteProps) {
    const { t } = useTranslation();

    // JSON Helper functions
    const addJsonMod = () => onChangeJsonModifications([...jsonModifications, { path: '', value: '', operation: 'set' }]);
            ...
            <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2 duration-200">
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                <label className={LABEL_STYLE}>{t('rule_editor.action.map_local.content_type')}</label>
                                <span className="text-[10px] text-muted-foreground/40 font-medium mb-1.5 uppercase tracking-tighter">{t('common.optional')}</span>
                            </div>
                            <Input
                                type="text"
                                value={contentType || ''}
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Input
                                ...
                                onChange={(e) => {
                                    ...
                                    onChangeStatusCode(val === '' ? undefined : parseInt(val));
                                }}
                                placeholder="200"
                                className="font-mono text-xs"
                            />
                ...
                {(type === 'json' && target === 'response') && (
                    <div className="hidden">
                        {/* Hidden block to remove previous injection if any */}
                    </div>
                )}
                    ...
                    <div className="space-y-3 pt-2 animate-in fade-in slide-in-from-top-2 duration-200">
                        <div className="space-y-2">
                            ...
                            {jsonModifications.map((mod, i) => (
                                <div key={i} className="flex gap-2 items-center p-2 bg-background/80 rounded-lg border border-border/50 shadow-sm">
                                    <button
                                    ...
                                    >
                                        {mod.enabled !== false ? <CheckCircle2 className="w-4 h-4" /> : <Circle className="w-4 h-4" />}
                                    </button>
                                    <div className="flex-1 flex gap-2 items-center">
                                        ...
                                        {mod.operation !== 'delete' && (
                                            <Input
                                                ...
                                                placeholder={t('rule_editor.action.rewrite.json_value_placeholder', 'Value (JSON)')}
                                                className="flex-1 h-7 text-[11px] font-mono px-2"
                                            />
                                        )}
                                    </div>
