...
import { UrlMatchType, HttpMethod } from '../../../types/rules';
    ...
    useEffect(() => {
        ...
        try {
            if (urlMatchType === 'exact') {
                ...
                const re = new RegExp('^' + urlPattern.replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*').replace(/\?/g, '.') + '$');
                setIsMatch(re.test(testUrl));
            }
        } catch (e) {
            setIsMatch(false);
            ...
            <div className="space-y-3">
                ...
                <div className="flex flex-col gap-1.5">
                    <div className="flex flex-col gap-2 p-3 bg-muted/20 rounded-xl border border-border/40">
                        <SegmentedControl
                            name="url-match-type"
                            options={[
                                { label: t('rule_editor.match.url_match_type.contains'), value: 'contains' },
                                { label: t('rule_editor.match.url_match_type.exact'), value: 'exact' },
                                { label: t('rule_editor.match.url_match_type.regex'), value: 'regex' },
                                { label: t('rule_editor.match.url_match_type.wildcard'), value: 'wildcard' },
                            ]}
                            value={urlMatchType}
                        ...
                        <div className="relative group/urlinput">
                            <Input
                                ...
                                className="h-8 font-mono text-system placeholder:font-sans placeholder:text-[11px] pr-10 bg-background/50"
                            />
                            {urlMatchType === 'regex' && (
                        ...
                        <div className="mt-1 space-y-1.5">
                            <div className="flex items-center justify-between">
                                ...
                                {testUrl && (
                                    <div className={`text-[11px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1.5 transition-all duration-300 ${isMatch ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                                        {isMatch ? <CheckCircle2 className="w-2.5 h-2.5" /> : <AlertCircle className="w-2.5 h-2.5" />}
                                        {isMatch ? t('rule_editor.match.test.matched') : t('rule_editor.match.test.not_matched')}
                                    </div>
                                )}
                            </div>
                            <Input
                                ...
                                className="h-8 bg-background/30 border-dashed border-input transition-all focus:border-solid text-system placeholder:text-[11px]"
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                    <div className="flex flex-wrap gap-2">
                        ...
                        {(['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS', 'HEAD'] as HttpMethod[]).map((method) => {
                ...
                <div className="pt-2">
                    ...
                    <div className="space-y-4 p-4 bg-muted/20 rounded-xl border border-border/40">
                                        ...
                                        <div className="flex-1 grid grid-cols-[1.5fr_1fr_2fr] gap-2">
                                            <div>
                                                <Input
                                                    ...
                                                    placeholder="Key"
                                                    className="h-8 py-1 px-2 text-system placeholder:text-xs font-mono w-full"
                                                />
                                            </div>
                                            <div>
                                                <Select
                                                ...
                                                >
                                                    <option value="exact">{t('rule_editor.match.url_match_type.exact')}</option>
                                                    <option value="contains">{t('rule_editor.match.url_match_type.contains')}</option>
                                                    <option value="regex">{t('rule_editor.match.url_match_type.regex')}</option>
                                                </Select>
                                            </div>
                                            <div>
                                                <Input
                                                    ...
                                                    placeholder="Value"
                                                    className="h-8 py-1 px-2 text-system placeholder:text-xs w-full"
                                                />
                                            </div>
                                        </div>
                                        <button
                        ...
                        <button
                            type="button"
                            onClick={() => onChangeHeaders([...requiredHeaders, { key: '', value: '', matchType: 'exact' }])}
                            className="w-full py-2 flex items-center justify-center gap-1.5 border border-dashed border-border rounded-lg text-xs font-medium text-muted-foreground hover:text-primary hover:border-primary/50 hover:bg-primary/5 transition-all"
                        >
