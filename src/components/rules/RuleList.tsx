...
import { Trash2, Edit, Globe, FileCode, Wifi, ShieldCheck, LayoutList, Ban, AlertTriangle, ChevronUp, ChevronDown, FileSignature } from 'lucide-react';
...
export function RuleList({ rules, onEdit, conflicts = {}, selectedRuleId }: RuleListProps) {
    ...
    const getRuleIcon = (type: RuleType) => {
        switch (type) {
            case 'rewrite_body': return <FileSignature className="w-3.5 h-3.5" />;
            case 'map_local': return <FileCode className="w-3.5 h-3.5" />;
            case 'map_remote': return <Globe className="w-3.5 h-3.5" />;
            case 'rewrite_header': return <LayoutList className="w-3.5 h-3.5" />;
            case 'throttle': return <Wifi className="w-3.5 h-3.5" />;
            case 'block_request': return <Ban className="w-3.5 h-3.5" />;
    ...
    const getRuleColorCheck = (type: RuleType) => {
        switch (type) {
            case 'rewrite_body': return 'text-purple-500 bg-purple-500/10 border-purple-200 dark:border-purple-900';
            case 'map_local': return 'text-blue-500 bg-blue-500/10 border-blue-200 dark:border-blue-900';
            case 'map_remote': return 'text-emerald-500 bg-emerald-500/10 border-emerald-200 dark:border-emerald-900';
            case 'rewrite_header': return 'text-orange-500 bg-orange-500/10 border-orange-200 dark:border-orange-900';
            case 'throttle': return 'text-cyan-500 bg-cyan-500/10 border-cyan-200 dark:border-cyan-900';
            case 'block_request': return 'text-rose-500 bg-rose-500/10 border-rose-200 dark:border-rose-900';
            default: return 'text-gray-500 bg-gray-500/10 border-gray-200 dark:border-gray-800';
        }
    };
    ...
    const belowLabel = (rule: Rule) => {
        ...
        switch (primaryAction.type) {
            case 'map_local':
                return t('rules.summary.map_local', {
                    path: primaryAction.localPath ? primaryAction.localPath.split(/[/\\]/).pop() : t('rules.summary.manual_mock')
                }) + actionCount;
            case 'map_remote':
                return t('rules.summary.map_remote', {
                ...
                {rules.map((rule, index) => {
                    ...
                    return (
                        <motion.div
                            ...
                            key={rule.id}
                            className={`group relative flex items-center px-3 py-1.5 rounded-2xl border transition-all duration-300 ${isSelected
                        ...
                        >
                            ...
                            <div className="pl-0.5 pr-2.5 relative z-10">
                                <Tooltip content={rule.execution.enabled ? t('common.disable') : t('common.enable')}>
                                    <Switch
                                        size="sm"
                            ...
                            {/* Icon Box */}
                            <div className={`relative z-10 flex items-center justify-center w-7 h-7 rounded-lg mr-2.5 ${colorClass.split(' ').filter(c => !c.includes('border')).join(' ')}`}>
                            ...
                            >
                                <div className="flex items-center gap-2 mb-0 min-h-[20px]">
                                    <span className={`font-semibold text-xs truncate transition-colors tracking-tight ${rule.execution.enabled
                                        ? isSelected ? 'text-primary' : 'group-hover/content:text-primary'
                                        : 'text-muted-foreground'
                                        }`}>{rule.name}</span>
                                    {conflict && rule.execution.enabled && (
                                        <Tooltip content={
                                            <div className="max-w-[200px] whitespace-normal space-y-1">
                                                <p className="font-semibold text-red-400">{t('rules.conflict.shadowed')}</p>
                                                <p className="text-[10px] opacity-80 leading-tight">
                                                    {t('rules.conflict.shadowed_desc', { name: allRules.find(r => r.id === conflict.byRuleId)?.name })}
                                                </p>
                                            </div>
                                        }>
                                            <div className="flex items-center gap-1 bg-destructive/10 text-destructive px-1.5 py-0.5 rounded-full border border-destructive/20 shadow-sm animate-pulse cursor-help pointer-events-auto">
                                                ...
                                                <span className="text-[9px] uppercase font-semibold leading-none">{t('rules.conflict.overridden')}</span>
                                            </div>
                                        </Tooltip>
                                    )}
                                    {!rule.execution.enabled && (
                                        <span className="text-[9px] uppercase font-semibold px-1.5 py-0.5 rounded-md bg-muted text-muted-foreground/80 leading-none">{t('common.disabled')}</span>
                                    )}
                                </div>
                                <div className="flex items-center gap-2 text-[12px] text-muted-foreground/80">
                                    <span className={`truncate opacity-100 ${conflict && rule.execution.enabled ? 'line-through decoration-destructive/40' : ''} ${isSelected ? 'text-primary/90' : ''}`}>
                                        {rule.match.request.find(m => m.type === 'url' || m.type === 'host')?.value || t('rules.match_all')}
                                    </span>
                                    {belowLabel(rule) && (
                                        <>
                                            <span className="opacity-30 text-[10px]">•</span>
                                            <span className="truncate text-[11px] text-muted-foreground/70">{belowLabel(rule)}</span>
                                        </>
                                    )}
                                </div>
                            </div>
