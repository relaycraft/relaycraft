...
export function RuleList({ rules, onEdit, conflicts = {}, selectedRuleId }: RuleListProps) {
  ...
  const belowLabel = (rule: Rule) => {
    ...
    // Prioritize map_local action for display in the summary
    const primaryAction = actions.find((a) => a.type === "map_local") || actions[0];

    const moreCount = actions.length - 1;
    const actionCountStr =
      moreCount > 0 ? t("rules.summary.more_actions", { count: moreCount }) : "";
    ...
    switch (primaryAction.type) {
      ...
      case "rewrite_body": {
        ...
        return t("rules.summary.rewrite_body", { target, type }) + actionCountStr;
      }
      case "throttle":
        return t("rules.summary.throttle", { delay: primaryAction.delayMs }) + actionCountStr;
      case "rewrite_header": {
        const reqCount = primaryAction.headers?.request?.length || 0;
        const resCount = primaryAction.headers?.response?.length || 0;
        return t("rules.summary.rewrite_header", { req: reqCount, res: resCount }) + actionCountStr;
      }
      case "block_request":
        return t("rules.summary.block") + actionCountStr;
      default:
        return "";
    }
  };
            ...
            >
              ...
              >
                <div className="flex items-center gap-2 mb-0 min-h-[20px]">
                  <span
                    className={`font-medium text-ui truncate transition-colors tracking-tight max-w-[90%] ${
