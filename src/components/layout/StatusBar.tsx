...
import { Server, Database, Octagon, Bell, BellOff, Globe, Activity } from 'lucide-react';
...
import { usePluginSlotStore, PLUGIN_SLOTS } from '../../stores/pluginSlotStore';
...
export function StatusBar() {
    ...
    useEffect(() => {
        ...
        const handler = (e: MouseEvent) => {
            ...
            if (!target.closest('.breakpoint-manager-trigger') && !target.closest('.breakpoint-manager-container')) {
    ...
    return (
        <div className="h-7 bg-primary/5 border-t border-border flex items-center justify-between px-3 text-xs select-none relative z-50">
            <div className="flex items-center gap-4">
                ...
                <Tooltip content={t('status_bar.ip')}>
                    <div className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors cursor-default">
                        ...
                        <span className="font-mono text-foreground">{useProxyStore.getState().ipAddress || '...'}</span>
                    </div>
                </Tooltip>
            ...
            <div className="flex items-center gap-4">
                ...
                {/* Notifications */}
                <Tooltip content={dnd ? t('notifications.dnd_on', 'Do Not Disturb On') : t('notifications.title', 'Notifications')}>
                    <div
