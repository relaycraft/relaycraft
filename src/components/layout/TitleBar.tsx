import { Square, Play, Search, Sparkles, Loader2 } from 'lucide-react';
...
export function TitleBar({ running, loading, onToggle }: TitleBarProps) {
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 flex justify-center max-w-2xl px-8 h-full items-center" data-tauri-drag-region>
                <button
                ...
                >
                    ...
                    <div className="flex-1 text-left text-xs text-muted-foreground/60 group-hover:text-foreground/80 transition-colors font-medium">
                        {t('titlebar.search_placeholder', { hotkey: isMacOS ? '⌘K' : 'Ctrl+K' })}
                    </div>
            ...
            <div className="flex items-center gap-3 w-56 justify-end" data-tauri-drag-region>
                ...
                <div className="flex items-center" onMouseDown={(e) => e.stopPropagation()}>
                    {onToggle && (
                        <Tooltip content={<span>{running ? t('titlebar.stop_proxy') : t('titlebar.start_proxy')} <span className="text-xs opacity-50 ml-1">⌘\</span></span>}>
                            <Button
                                onClick={onToggle}
                                disabled={loading}
                                variant={running ? "destructive" : "secondary"}
                                size="xs"
                                className={`gap-1.5 h-7 min-w-[72px] relative overflow-hidden transition-all duration-300 rounded-lg ${running
                            ...
                            >
                                ...
                                <span className="text-xs font-bold tracking-tight">
                                    {loading ? (running ? t('titlebar.stop_proxy') : t('titlebar.start_proxy')) + "..." : (running ? t('titlebar.running') : t('titlebar.stopped'))}
                                </span>
                            </Button>
                        </Tooltip>
                    )}
                ...
                {!isMacOS && (
                    <div className="flex items-center h-full">
                        ...
                        <div className="flex items-center" onMouseDown={(e) => e.stopPropagation()}>
                            ...
                            <Tooltip content={isMaximized ? t('titlebar.restore') : t('titlebar.maximize')} side="bottom">
                                <button
                                    className="h-8 w-10 flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-white/5 transition-colors"
                                ...
                                >
                                    {isMaximized ? (
                                        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <path d="M3.5 1H8.5C8.77614 1 9 1.22386 9 1.5V6.5C9 6.77614 8.77614 7 8.5 7H7" stroke="currentColor" strokeWidth="1" strokeLinejoin="round" strokeLinecap="round" />
