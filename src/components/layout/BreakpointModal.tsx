...
import { Play, AlertOctagon, ArrowRightLeft, Globe, Hash, Layers, Trash2, CheckCircle2, X } from 'lucide-react';
...
export function BreakpointModal({ flows, onClose, onResume }: BreakpointModalProps) {
    ...
    useEffect(() => {
        ...
        setHeaders(Object.entries(sourceHeaders).map(([key, value]) => ({
            key,
            value,
            enabled: true
        })));
                ...
                <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="w-56 border-r border-border bg-muted/5 flex flex-col overflow-hidden">
                        <div className="p-2.5 border-b border-border bg-muted/10">
                            <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest pl-1">{t('breakpoint.queue')}</span>
                        </div>
                        <div className="flex-1 overflow-y-auto p-1.5 space-y-0.5 no-scrollbar">
                            {flows.map((f) => (
                                <button
                                    ...
                                    className={`w-full text-left p-2.5 rounded-xl transition-all group relative overflow-hidden ${selectedFlowId === f.id
                                        ? 'bg-primary/10 border border-primary/20'
                                        : 'hover:bg-muted/40 border border-transparent'
                                        }`}
                                >
                                    <div className="flex items-center justify-between mb-1">
                                        <span className={`text-[8px] font-black px-1 py-0.5 rounded ${f.interceptPhase === 'request' ? 'bg-blue-500/10 text-blue-600' : 'bg-green-500/10 text-green-600'
                                            }`}>
                                            {f.method}
                                        </span>
                                        <span className="text-[8px] font-mono text-muted-foreground opacity-40">#{f.id.slice(-4)}</span>
                                    </div>
                                    <div className={`text-[11px] font-mono truncate ${selectedFlowId === f.id ? 'text-primary font-bold' : 'text-foreground/60'}`}>
                                        {f.url}
                                    </div>
                                </button>
                            ))}
                        ...
                        <div className="px-5 py-2.5 border-b border-border/40 flex items-center justify-between bg-muted/5">
                            <div className="flex items-center gap-2.5 min-w-0 flex-1">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2">
                                        <span className={`px-1.5 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider border shrink-0 ${isRequest ? 'bg-blue-500/10 text-blue-600 border-blue-500/20' : 'bg-green-500/10 text-green-600 border-green-500/20'}`}>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <div className="flex items-center gap-2 pl-1">
                                        ...
                                        <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">{t('breakpoint.method_protocol')}</span>
                                    </div>
