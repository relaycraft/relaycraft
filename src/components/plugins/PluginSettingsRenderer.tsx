...
export const PluginSettingsRenderer: React.FC<PluginSettingsRendererProps> = ({
    ...
    if (!schema || !schema.properties) {
        return <div className="text-muted-foreground text-sm italic">{t('plugins.settings.no_schema', 'No configuration available for this plugin.')}</div>;
    }
    ...
    return (
        <div className={cn("space-y-4", className)}>
            {schema.description && <p className="text-[11px] text-muted-foreground/60 mb-2 leading-relaxed px-1">{schema.description}</p>}
            ...
            <div className="space-y-2.5">
                {Object.entries(schema.properties).map(([key, prop]) => {
                    ...
                    const displayTitle = t(`${namespace}:${titleKey}`, { defaultValue: prop.title || key });
                    const displayDesc = t(`${namespace}:${descKey}`, { defaultValue: prop.description });
                        ...
                        <div key={key} className="space-y-2">
                            ...
                            {prop.type === 'boolean' && (
                                <div className="flex items-center justify-between p-3.5 rounded-xl bg-secondary/30 hover:bg-secondary/50 transition-all border border-border/5">
                                    <div className="space-y-0.5">
                                        <Label htmlFor={key} className="text-system font-semibold text-foreground/90 cursor-pointer">
                                            {displayTitle}
                                        </Label>
                                        {displayDesc && (
                                            <p className="text-[10px] text-muted-foreground/60 leading-relaxed">{displayDesc}</p>
                                        )}
                                    </div>
                                    <Switch
                                        id={key}
                            ...
                            {prop.type !== 'boolean' && (
                                <div className="space-y-2.5 bg-secondary/30 p-3.5 rounded-xl border border-border/5 transition-all">
                                    <div className="space-y-0.5">
                                        <Label htmlFor={key} className="flex items-center gap-2 text-system font-semibold text-foreground/90">
                                            {displayTitle}
                                            {schema.required?.includes(key) && <span className="text-destructive">*</span>}
                                        </Label>
                                        {displayDesc && (
                                            <p className="text-[10px] text-muted-foreground/50 leading-relaxed">{displayDesc}</p>
                                        )}
                                    </div>
