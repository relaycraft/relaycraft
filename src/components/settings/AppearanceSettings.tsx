...
import { Globe, Monitor, Moon, Palette, Plus, Sun } from "lucide-react";
...
export const AppearanceSettings: React.FC = () => {
  ...
  const modes: { id: ThemeMode; icon: any; label: string }[] = [
    ...
    { id: "dark", icon: Moon, label: t("settings.appearance.mode_dark") },
    {
      id: "system",
      icon: Monitor,
      label: t("settings.appearance.mode_system"),
    },
    {
      id: "custom",
      icon: Palette,
      label: t("settings.appearance.mode_custom"),
    },
  ];
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              {themes.filter((t) => t.pluginId !== "system").length > 0 ? (
                ...
                <div
                  className="col-span-3 py-6 px-5 border border-border rounded-xl bg-gradient-to-br from-muted/30 to-muted/10 flex items-center justify-between group cursor-pointer hover:border-primary/20 transition-all"
                  onClick={() => setMarketOpen(true, "theme")}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      e.preventDefault();
                      setMarketOpen(true, "theme");
                    }
                  }}
                  role="button"
                  tabIndex={0}
        ...
        <div className="p-4 flex gap-2">
          {["compact", "comfortable", "relaxed"].map((d) => (
            <Button
              ...
              size="sm"
              className={`flex-1 text-[11px] font-semibold h-9 transition-all duration-200 ${
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 backdrop-blur-[1px]">
          <div className="bg-popover border border-border rounded-xl shadow-lg p-6 max-w-sm w-full space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">{t("common.confirm_delete_desc")}</p>
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" size="sm" onClick={() => setThemeToDelete(null)}>
                {t("common.cancel")}
              </Button>
              <Button
                variant="destructive"
