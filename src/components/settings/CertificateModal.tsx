...
export function CertificateModal({ isOpen, onClose }: CertificateModalProps) {
    ...
    const windowsSteps = (t('cert.manual.guides.windows', { returnObjects: true }) as string[]).map((step: string) => <span dangerouslySetInnerHTML={{ __html: step }} />);
    const macosSteps = (t('cert.manual.guides.macos', { returnObjects: true }) as string[]).map((step: string) => <span dangerouslySetInnerHTML={{ __html: step }} />);
                    ...
                    <Tabs defaultValue="desktop" className="flex-1 flex flex-col h-full">
                        <div className="flex-none px-5 pt-4 pb-2">
                            <TabsList className="grid w-full grid-cols-2 bg-muted/50 p-1 rounded-lg h-auto border border-border/40">
                                <TabsTrigger value="desktop" className="rounded-md py-2 text-xs font-bold data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
                                    <div className="flex items-center justify-center gap-2">
                                        <Monitor className="w-3.5 h-3.5" />
                                        {t('cert.manual.tabs.desktop')}
                                    </div>
                                </TabsTrigger>
                                <TabsTrigger value="mobile" className="rounded-md py-2 text-xs font-bold data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
                                    <div className="flex items-center justify-center gap-2">
                                ...
                                <Tabs defaultValue={isMacOS ? "macos" : "windows"} className="flex flex-col h-full">
                                    <div className="flex-none mb-4 flex justify-start sticky top-0 bg-card z-10 py-2">
                                        <TabsList className="bg-muted/30 p-0.5 border border-border/40 w-fit h-auto gap-0.5 rounded-lg">
                                            <TabsTrigger value="windows" className="px-4 py-1.5 rounded-md text-[11px] font-medium transition-all">{t('cert.manual.desktop.windows')}</TabsTrigger>
                                            <TabsTrigger value="macos" className="px-4 py-1.5 rounded-md text-[11px] font-medium transition-all">{t('cert.manual.desktop.macos')}</TabsTrigger>
                                            <TabsTrigger value="linux" className="px-4 py-1.5 rounded-md text-[11px] font-medium transition-all">{t('cert.manual.desktop.linux')}</TabsTrigger>
                                        </TabsList>
                                    </div>
                                            ...
                                            <div className="space-y-5">
                                                ...
                                                <div className="pt-4 border-t border-border/40">
                                                    ...
                                                    <div className="bg-zinc-950 rounded-lg p-3 group relative border border-white/5 ring-1 ring-white/5 shadow-sm">
                                                        <div className="font-mono text-[10px] text-zinc-300 break-all pr-8 leading-relaxed">
                                                            sudo security add-trusted-cert -d -r trustRoot -p ssl -p basic -k /Library/Keychains/System.keychain "{certPath || '~/.mitmproxy/mitmproxy-ca-cert.pem'}"
                                                        </div>
                                                    ...
                                                    {[
                                                        ...
                                                        <div key={distro.id} className="space-y-1.5">
                                                            ...
                                                            <div className="bg-zinc-950 rounded-lg p-3 group relative border border-white/5 shadow-sm">
                                                                <div className="font-mono text-[10px] text-zinc-300 break-all pr-8 leading-relaxed">
                                                                    {distro.cmd.replace('{{path}}', certPath || '~/relaycraft-ca-cert.pem')}
                                                                </div>
                                                                <div className="absolute right-2 top-2">
                                                                    <CopyButton
                                                                        text={distro.cmd.replace('{{path}}', certPath || '~/relaycraft-ca-cert.pem')}
                                                                        className="p-1.5 bg-white/5 hover:bg-white/10 text-white rounded transition-all border border-white/10 active:scale-95"
                                                                        label={t('common.copy')}
                                                                    />
                                                                </div>
                                    ...
                                    <div className="space-y-3 pb-4">
                                        ...
                                        <Tabs defaultValue="ios" className="w-full">
                                            <TabsList className="bg-muted/20 p-0.5 border border-border/40 w-fit justify-start h-auto gap-0.5 rounded-lg mb-3">
                                                <TabsTrigger value="ios" className="px-4 py-1 rounded-md text-[10px] font-medium transition-all">iOS / iPadOS</TabsTrigger>
                                                <TabsTrigger value="android" className="px-4 py-1 rounded-md text-[10px] font-medium transition-all">Android</TabsTrigger>
                                            </TabsList>
        ...
        <div className="pl-1 pt-1">
            {steps.map((step, i) => (
                <div key={i} className="relative pl-10 pb-5 last:pb-0 group">
                    ...
                    {/* Badge */}
                    <div className="absolute left-1 top-1 w-7 h-7 rounded-lg bg-muted text-foreground/70 group-hover:bg-primary group-hover:text-primary-foreground flex items-center justify-center text-[10px] font-black shadow-sm transition-all duration-300 ring-4 ring-background z-10 border border-border/10">
                        {i + 1}
                    </div>

                    {/* Content */}
                    <div className="text-[12px] leading-relaxed pt-1.5 text-foreground/90 group-hover:text-foreground transition-colors font-medium">
                        {step}
                    </div>
                </div>
            ))}
