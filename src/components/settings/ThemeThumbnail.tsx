...
export const ThemeThumbnail: React.FC<ThemeThumbnailProps> = ({ theme, isActive, onClick, onDelete }) => {
    ...
    return (
        <div
            onClick={onClick}
            className={`group relative cursor-pointer rounded-xl border-2 transition-all p-2 bg-black/20 hover:bg-black/40 ${isActive ? 'border-primary ring-2 ring-primary/20' : 'border-transparent'
                }`}
        >
            {onDelete && (
                <div
            ...
            <div className="mt-2 flex items-center justify-between px-1">
                <span className="text-[10px] font-semibold opacity-80 truncate">{theme.name}</span>
                {isActive && (
