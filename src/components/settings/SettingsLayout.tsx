...
export function SettingsTabButton({
    ...
    return (
        <button
            onClick={onClick}
            className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-system font-semibold transition-all ${active
...
export function SettingsSection({ title, children, action }: { title: string; children: ReactNode; action?: ReactNode }) {
    return (
        <section>
...
export function SettingsRow({ title, description, children, onClick, className = "", danger = false, icon }: SettingsRowProps) {
    return (
        <div
            onClick={onClick}
            className={`group px-4 py-2.5 min-h-[56px] flex items-center justify-between hover:bg-muted/30 transition-all ${onClick ? 'cursor-pointer active:bg-muted/50' : ''
                } ${className}`}
        >
            <div className="flex-1 pr-4 flex items-center gap-3">
                ...
                <div>
                    <div className={`text-system font-semibold ${danger ? 'text-destructive' : 'text-foreground'}`}>
                        {title}
                    </div>
                    {description && (
