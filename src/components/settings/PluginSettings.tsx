...
const PluginCard: React.FC<{ plugin: PluginInfo }> = ({ plugin }) => {
    ...
    const isUI = !!plugin.manifest.capabilities?.ui?.entry || !!plugin.manifest.entry?.ui || !!plugin.manifest.capabilities?.i18n;
    ...
    }, [showSettings, hasSettings, plugin.manifest.id, loadSchema, loadSettings]);
    ...
    return (
        <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className={`group relative p-3 rounded-2xl border transition-all ${plugin.enabled
                ? 'bg-card border-primary/20 shadow-lg shadow-primary/5'
                : 'bg-muted/5 border-border/30 opacity-70 hover:opacity-100'
                }`}
        >
            <div className="flex items-start gap-3">
                <div className={`flex items-center justify-center w-9 h-9 rounded-xl shrink-0 ${plugin.enabled ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground/40'
                    }`}>
                        ...
                        <div className="flex items-center gap-1.5">
                            {hasSettings && (
                                <button
                                    ...
                                    className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${showSettings
                            ...
                            <button
                                ...
                                className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${plugin.enabled
                                    ? 'bg-muted/50 text-muted-foreground hover:bg-muted hover:text-foreground'
                                    : 'bg-muted/50 text-muted-foreground hover:bg-muted hover:text-foreground'
                                    }`}
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
                        {plugin.manifest.author && (
                            <div className="flex items-center gap-1 text-[10px] text-muted-foreground/60 font-medium">
                                ...
                                <span>{typeof plugin.manifest.author === 'string' ? plugin.manifest.author : plugin.manifest.author.name}</span>
                            </div>
                        )}
                        <div className="flex items-center gap-1 text-[10px] text-muted-foreground/60 font-medium">
                            <Code className="w-3 h-3" />
                            <span className="flex gap-1">
                                {isPython && <span className="px-1 py-0.5 rounded bg-muted/50 border border-border/50">Python</span>}
                                {isUI && <span className="px-1 py-0.5 rounded bg-muted/50 border border-border/50">UI</span>}
                            </span>
                        </div>
    ...
    const sortedPlugins = React.useMemo(() => {
        return [...plugins]
            .filter(p => {
                ...
                return name.includes(query) || desc.includes(query) || p.manifest.id.toLowerCase().includes(query);
            })
            .sort((a, b) => {
                // 1. Enabled First
                ...
                // 2. Alphabetical
                const nameA = (a.manifest.locales?.[i18n.language?.split('-')[0]]?.name || a.manifest.name).toLowerCase();
                const nameB = (b.manifest.locales?.[i18n.language?.split('-')[0]]?.name || b.manifest.name).toLowerCase();
                ...
                <div className="p-4 space-y-4">
                    ...
                    {loading && plugins.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-muted-foreground/40 gap-4">
                            ...
                            <span className="text-xs font-medium uppercase tracking-widest">{t('plugins.scanning')}</span>
                        </div>
                    ...
                    <div className="mt-4 p-4 bg-primary/5 border border-primary/10 rounded-2xl flex gap-3">
                        ...
                        <div className="space-y-1">
                            <h5 className="text-[11px] text-primary font-bold uppercase tracking-widest">{t('plugins.guide.title')}</h5>
                            <p className="text-[11px] text-muted-foreground leading-relaxed">
                                <Trans i18nKey="plugins.guide.desc" components={{
