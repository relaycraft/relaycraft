...
import { Send, X, Code2, Globe, AlertCircle, Eraser, Download, Zap, Loader2 } from 'lucide-react';
...
export function ComposerView() {
    ...
    const [result, setResult] = useState<{ type: 'error', message: string } | null>(null);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (builderMatch) {
                localizedMessage = t('composer.url_builder_error', { url: builderMatch[1] });
            } else if (sendingMatch) {
                localizedMessage = t('composer.error_sending_request', { url: sendingMatch[1] });
    ...
    const handleParseCurl = () => {
        ...
        if (parsed) {
            ...
            setUrl(parsed.url);
            setHeaders(Object.entries(parsed.headers).map(([key, value]) => ({ key, value, enabled: true })));
            setBody(parsed.body || '');
                ...
                <div className="flex items-center gap-1 p-1 bg-muted/20 backdrop-blur-xl border border-border/40 rounded-lg shadow-sm group focus-within:ring-1 ring-primary/20 transition-all">
                    ...
                    <div className="flex items-center gap-1 pr-1 shrink-0">
                        <Tooltip content={t('composer.parse_curl_tooltip')}>
                            <Button variant="quiet" size="icon-sm" onClick={() => setShowCurlModal(true)} className="h-7 w-7 rounded-lg text-muted-foreground">
                                <Code2 className="w-3.5 h-3.5" />
                            </Button>
                        </Tooltip>
                        <Tooltip content={t('composer.reset')}>
                            <Button variant="quiet" size="icon-sm" onClick={reset} className="h-7 w-7 rounded-lg hover:text-destructive group/reset">
                    ...
                    <Tabs defaultValue="headers" className="flex-1 flex flex-col overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-muted/10">
                            <TabsList className="bg-muted/20 p-0.5 rounded-lg border border-border/20 h-8">
                                <TabsTrigger value="headers" className="h-7 px-4 text-xs font-bold rounded-md data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm">
                                    {t('composer.headers')}
                                </TabsTrigger>
                                <TabsTrigger value="body" className="h-7 px-4 text-xs font-bold rounded-md data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm">
                                    {t('composer.body')}
                                </TabsTrigger>
                            </TabsList>
                        ...
                        <div className="flex-1 overflow-hidden">
                            <TabsContent value="headers" className="m-0 p-4 animate-in fade-in slide-in-from-left-2 duration-300 h-full overflow-y-auto no-scrollbar">
                                <HeaderListEditor headers={headers} onChange={setHeaders} />
                            </TabsContent>

                            <TabsContent value="body" className="m-0 p-4 animate-in fade-in slide-in-from-right-2 duration-300 h-full flex flex-col overflow-y-auto no-scrollbar">
                                {bodyType === 'none' ? (
                                    <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground/30">
                                        ...
                                        <p className="text-[10px] font-bold uppercase tracking-widest">{t('composer.body_none')}</p>
                                    </div>
                                            ...
                                            <div className="absolute right-3 bottom-3 opacity-0 group-hover:opacity-100 transition-opacity z-20 flex gap-2 pointer-events-none">
                                                <div className="pointer-events-auto">
                                                    <Tooltip content={t('common.format')}>
                                                        <Button variant="quiet" size="icon-sm" onClick={handleFormatJson} className="h-8 w-8 bg-background/80 backdrop-blur rounded-lg border border-border/20 shadow-sm">
                ...
                <div className="flex-1 flex flex-col bg-card/20 backdrop-blur-md rounded-2xl border border-white/5 overflow-hidden shadow-sm relative group/response">
                    <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-muted/10 flex-shrink-0">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-foreground/60">{t('composer.response')}</span>
                        </div>

                        {lastResponse && (
                            <div className="flex items-center gap-3">
                                <span className={cn("px-2 py-0.5 rounded-md text-[10px] font-black border", getStatusColor(lastResponse.status || 0))}>
                                    {lastResponse.status}
                                </span>
                                ...
                                >
                                    ...
                                    <div className="absolute right-4 bottom-4 flex items-center gap-2 opacity-0 group-hover/response:opacity-100 transition-opacity">
                                        ...
                                        <Tooltip content={t('content_preview.download')}>
                                            <Button variant="quiet" size="icon-sm" onClick={handleDownload} className="h-9 w-9 bg-background/90 backdrop-blur rounded-xl border border-white/10 shadow-lg">
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-8 bg-black/25 backdrop-blur-[1px] animate-in fade-in duration-300">
                    ...
                    >
                        <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-muted/20">
                            <h3 className="text-sm font-bold tracking-tight text-foreground">{t('composer.curl_modal.title')}</h3>
                            <button onClick={() => { setShowCurlModal(false); setParseError(null); }} className="text-muted-foreground/40 hover:text-foreground transition-colors"><X className="w-5 h-5" /></button>
                        </div>
                        <div className="p-6 space-y-4">
                            <p className="text-[11px] text-muted-foreground/80 leading-relaxed font-medium">{t('composer.curl_modal.desc')}</p>
                            <div className="relative group">
                                <Textarea
                                    ...
                                    className={cn(
                                        ...
                                        parseError ? "border-red-500/50 focus:ring-red-500/20" : "border-white/5 focus:ring-primary/20"
                                    )}
                                />
                                {parseError && (
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                <Button
                                    variant="quiet"
                                    onClick={() => { setShowCurlModal(false); setParseError(null); }}
