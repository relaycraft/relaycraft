...
async function loadPluginLocales(
    ...
    const languagesToLoad = specificLang
        ? [specificLang]
        : Array.from(new Set([currentLang, 'en']));

    Logger.debug(`[PluginLoader] Loading locales for ${pluginId} (NS: ${targetNamespace}). Targets: ${languagesToLoad.join(',')}`);
    ...
    for (const langCode of Object.keys(locales)) {
        ...
        useUIStore.getState().registerAvailableLanguage(
            langCode,
...
export function unloadPluginUI(pluginId: string) {
    ...
    const pluginLangs = useUIStore.getState().availableLanguages.filter(l => l.pluginId === pluginId);
