...
export function matchFlow(
	...
	const checkGroup = (
		items: FilterItem[],
		matchFn: (item: FilterItem) => boolean,
	) => {
		...
		if (negative.length > 0) {
			...
			if (negative.some((i) => matchFn({ ...i, negative: false })))
				return false;
		}
	...
	if (
		!checkGroup(
			criteria.method,
			(item) => flow.method.toLowerCase() === item.value,
		)
	)
		return false;
	...
	if (
		!checkGroup(criteria.ip, (item) => {
			return (
				...
				(flow.serverIp || "").includes(item.value)
			);
		})
	)
		return false;
	...
	if (
		!checkGroup(criteria.header, (item) => {
			...
			return Object.entries(combined).some(
				([k, v]) =>
					k.toLowerCase().includes(item.value) ||
					String(v).toLowerCase().includes(item.value),
			);
		})
	)
		return false;
	...
	if (
		!checkGroup(criteria.body, (item) =>
			matchText(flow.responseBody, item.value),
		)
	)
		return false;
	if (
		!checkGroup(criteria.reqbody, (item) =>
			matchText(flow.requestBody, item.value),
		)
	)
		return false;
