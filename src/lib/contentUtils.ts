export type ContentType = 'image' | 'json' | 'html' | 'javascript' | 'css' | 'xml' | 'text' | 'binary' | 'unknown';

export function detectContentType(headers: Record<string, string> | null, content: string | null): ContentType {
    if (!headers) return 'text';

    // Case-insensitive header lookup
    const contentTypeHeader = Object.keys(headers).find(key => key.toLowerCase() === 'content-type');
    const contentTypeValue = contentTypeHeader ? headers[contentTypeHeader].toLowerCase() : '';
    ...
    if (contentTypeValue.includes('text/javascript') || contentTypeValue.includes('application/javascript')) return 'javascript';
    if (contentTypeValue.includes('text/css')) return 'css';
    if (contentTypeValue.includes('text/xml') || contentTypeValue.includes('application/xml')) return 'xml';
...
export function getExtensionFromHeaders(headers: Record<string, string> | null, contentType: ContentType): string {
    if (!headers) return 'txt';

    const contentTypeHeader = Object.keys(headers).find(key => key.toLowerCase() === 'content-type');
    const mime = contentTypeHeader ? headers[contentTypeHeader].toLowerCase() : '';

    if (mime.includes('application/json')) return 'json';
    if (mime.includes('text/html')) return 'html';
    if (mime.includes('text/javascript') || mime.includes('application/javascript')) return 'js';
    if (mime.includes('text/css')) return 'css';
    if (mime.includes('text/xml') || mime.includes('application/xml')) return 'xml';
    ...
    switch (contentType) {
        ...
        case 'css': return 'css';
        case 'image': return 'png'; // default image
