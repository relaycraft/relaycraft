import { Rule, MapLocalAction, MapRemoteAction, RewriteBodyAction, ThrottleAction } from '../types/rules';
...
export function generateScriptFromRule(rule: Rule): string {
    ...
    const hasJsonAction = actions.some(a => a.type === 'rewrite_body' && a.json);
...
` : '';
    ...
    actions.forEach(action => {
        if (action.type === 'map_local') {
            ...
            const reqSnippet = generateHeaderOperationsSnippet('request', action.headers.request, indent + indent);
            const resSnippet = generateHeaderOperationsSnippet('response', action.headers.response, indent + indent);
    ...
    atoms.forEach(atom => {
        ...
        if (atom.type === 'url') {
            ...
            if (atom.matchType === 'exact' || atom.matchType === 'equals') {
                ...
                const regex = val.replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*').replace(/\?/g, '.');
...
function generateMapLocalSnippet(action: MapLocalAction, indentStr: string): string {
    ...
    if (action.source === 'manual') {
        const content = (action.content || '').replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, '\\n');
...
function generateMapRemoteSnippet(action: MapRemoteAction, indentStr: string): string {
...
function generateHeaderOperationsSnippet(target: 'request' | 'response', operations: { operation: string, key: string, value?: string }[], indent: string): string {
...
function generateRewriteBodySnippet(action: RewriteBodyAction, indentStr: string): string {
    const event = action.target === 'request' ? 'request' : 'response';
    let bodyLogic = '';

    if (action.set) {
        const escapedContent = (action.set.content || '').replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, '\\n');
...
${(action.json.modifications || []).filter(m => m.enabled !== false).map(m => generateJsonModification(m, 'data', innerIndent)).join('\n')}
...
function generateJsonModification(mod: { path: string, operation: string, value?: any }, varName: string, indentStr: string): string {
...
function generateThrottleSnippet(action: ThrottleAction, indentStr: string): string {
