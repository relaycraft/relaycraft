...
export function mapAIRuleToInternal(aiRule: AIRule): Partial<Rule> {
    ...
    if (Array.isArray(aiRule.actions)) {
        baseRule.actions = aiRule.actions.map(a => {
            ...
            if (action.type === 'rewrite_body' && action.rewriteType && !action.set && !action.replace && !action.regex_replace && !action.json) {
                const rt = action.rewriteType;
                if (rt === 'set') {
                    action.set = { content: action.content || '', statusCode: action.statusCode, contentType: action.contentType };
                } else if (rt === 'replace') {
                    action.replace = { pattern: action.pattern || '', replacement: action.replacement || '' };
                }
    ...
    if (!baseRule.match || (baseRule.match.request.length === 0 && baseRule.match.response.length === 0)) {
