    ...
    getSuggestions(context: SuggestionContext, t: (key: string, options?: any) => string): Suggestion[] {
        ...
        if (isTrafficTab) {
            if (context.selectedFlow) {
                ...
                suggestions.push({
                    id: 'flow_analysis',
                    label: t('command_center.suggestions.traffic_analyze', { method: context.selectedFlow.method }),
                    action: t('command_center.suggestions.traffic_analyze', { method: context.selectedFlow.method }),
                    group: 'ai',
                    score: 100
                });
                suggestions.push({
