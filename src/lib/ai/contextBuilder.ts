...
import { AIContext, AIContextOptions } from '../../types/ai';
...
const summarizeRule = (rule: Rule): { match: string; action: string } => {
    // Basic match summary
    const matches = rule.match.request.map(atom => {
        if (atom.type === 'url') return `${atom.matchType === 'regex' ? 'Regex' : 'URL'}: ${atom.value}`;
...
export const buildAIContext = async (options: AIContextOptions = {}): Promise<AIContext> => {
    const { includeLogs = false, includeHeaders = false, includeBody = false, maxTrafficCount = 5 } = options;
    ...
    const recentTraffic = flows.slice(-maxTrafficCount).map(f => ({
        id: f.id,
        method: f.method,
    ...
    if (activeTab === 'traffic' && selectedFlow) {
        selectedItem = {
            ...
            details: {
                ...
                requestHeaders: includeHeaders ? sanitizeHeaders(selectedFlow.requestHeaders) : undefined,
                responseHeaders: includeHeaders ? sanitizeHeaders(selectedFlow.responseHeaders) : undefined,
                requestBody: includeBody ? truncate(selectedFlow.requestBody || "", 500) : undefined,
                responseBody: includeBody ? truncate(selectedFlow.responseBody || "", 500) : undefined,
            }
        };
