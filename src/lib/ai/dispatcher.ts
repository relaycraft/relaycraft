...
function detectContextOptions(
	...
	if (
		inputLower.match(/header|头部|body|主体|内容|json|分析|analyze|content/)
	) {
...
export async function dispatchCommand(
	...
	const intentSystemMsg: AIMessage = {
		...
		content: GLOBAL_COMMAND_SYSTEM_PROMPT.replace(
			/{{LANGUAGE}}/g,
			langInfo.name,
		)
	...
	try {
		...
		if (
			action.intent === "CHAT" ||
			(action.intent === "CREATE_SCRIPT" && action.params?.requirement)
		) {
...
async function runTwoStageChat(
	...
	const systemPrompt = isScriptIntent
		? MITMPROXY_SYSTEM_PROMPT
		: CHAT_RESPONSE_SYSTEM_PROMPT;
