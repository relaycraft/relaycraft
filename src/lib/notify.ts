...
const inferCategoryAndPriority = (type: NotificationType, message: string): { category: NotificationCategory; priority: NotificationPriority } => {
    // 根据消息内容推断分类
    ...
    if (lowerMessage.includes('plugin') || lowerMessage.includes('插件')) {
        ...
        category = 'script';
    } else if (lowerMessage.includes('network') || lowerMessage.includes('网络') || lowerMessage.includes('request') || lowerMessage.includes('请求')) {
        category = 'network';
    } else if (lowerMessage.includes('update') || lowerMessage.includes('更新') || lowerMessage.includes('version') || lowerMessage.includes('版本')) {
        category = 'update';
    } else if (lowerMessage.includes('certificate') || lowerMessage.includes('证书') || lowerMessage.includes('security') || lowerMessage.includes('安全')) {
        category = 'security';
    }

    // 推断优先级
    if (type === 'error') {
        priority = lowerMessage.includes('crash') || lowerMessage.includes('崩溃') || lowerMessage.includes('critical') ? 'critical' : 'high';
...
const trigger = (type: NotificationType, message: string, options?: string | NotifyOptions) => {
    ...
    if (!toastOnly) {
        addNotification({
            title: title || i18n.t(`common.${type}`, { defaultValue: type.charAt(0).toUpperCase() + type.slice(1) }),
            message,
            type,
            category,
            priority,
