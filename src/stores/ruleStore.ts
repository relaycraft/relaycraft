...
interface RuleStore {
    ...
    exportBundle: () => string;
    importBundle: (json: string) => { success: boolean; count?: number; error?: string };
    exportRulesZip: (savePath: string) => Promise<{ success: boolean; message?: string; error?: string }>;
    importRulesZip: (zipPath: string) => Promise<{ success: boolean; imported?: number; skipped?: number; error?: string }>;
...
export const useRuleStore = create<RuleStore>((set, get) => ({
    ...
    toggleRule: (id) => {
        set((state) => {
            ...
            const updates: Partial<RuleStore> = {
                rules: state.rules.map((rule) =>
                    rule.id === id ? { ...rule, execution: { ...rule.execution, enabled: nextEnabled } } : rule
                )
            };
    ...
    moveRule: (id: string, direction: 'up' | 'down') => {
        set((state) => {
            ...
            const groupRules = state.rules
                ...
                .sort((a, b) => {
                    if (a.execution.priority !== b.execution.priority) return a.execution.priority - b.execution.priority;
        ...
        set((state) => {
            ...
            return {
                groups: state.groups.map((g) =>
                    g.id === id ? { ...g, enabled: nextEnabled } : g
                ),
                // Cascade enablement to rules in this group
                rules: state.rules.map(r =>
                    state.ruleGroups[r.id] === id ? { ...r, execution: { ...r.execution, enabled: nextEnabled } } : r
                )
            };
        });
        get().saveRules();
    ...
    loadRules: async () => {
        try {
            ...
            const response: { rules: { groupId: string, rule: Rule }[], errors: { path: string, error: string }[] } = JSON.parse(rulesJson);
            ...
            const ruleGroups = response.rules.reduce((acc, e) => {
            ...
            (bundle.rules || []).forEach((r: Rule) => {
                ...
                if (sourceGroupId) {
                    ...
                    if (mappedId) {
                        ...
                        const existingGroup = currentGroups.find(g => g.id === sourceGroupId || g.name === sourceGroupId);
                        if (existingGroup) {
    ...
    importRulesZip: async (zipPath: string) => {
        try {
            const rulesDir = await invoke<string>('get_rules_dir_path');
            const result = await invoke<{ success: boolean; importedCount: number; skippedCount: number; error?: string }>('import_rules_zip', {
                zipPath,
                rulesDir
            });
...
function isSuperset(ruleA: Rule, ruleB: Rule): boolean {
    ...
    for (const [type, listA] of mapA.entries()) {
        ...
        if (type === 'method') {
            const methodsA = new Set(listA.flatMap(a => Array.isArray(a.value) ? a.value : [a.value]));
            const methodsB = new Set(listB.flatMap(b => Array.isArray(b.value) ? b.value : [b.value]));
...
export function getRuleConflicts(rules: Rule[], groups: RuleGroup[]) {
    ...
    for (const rule of activeRules) {
        ...
        const hasRequestAction = actions.some(a =>
            ['map_local', 'map_remote', 'rewrite_header', 'rewrite_body', 'throttle', 'block_request'].includes(a.type) &&
        ...
        if (!conflicts[rule.id]) {
            const hasResponseAction = actions.some(a =>
