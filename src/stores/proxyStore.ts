...
export const useProxyStore = create<ProxyStore>((set) => ({
    ...
    startProxy: async () => {
        try {
            ...
            const status = await invoke<{ running: boolean; active_scripts: string[] }>('get_proxy_status');
            if (status.running) {
    ...
    checkStatus: async () => {
        try {
            ...
            if (status.running === currentState.running && currentState.ipAddress && currentState.port !== 9090) {
            ...
            const ipAddress = await invoke<string>('get_local_ip');
            const config = await invoke<{ proxy_port: number; cert_warning_ignored: boolean }>('load_config');
    ...
    checkCertTrust: async () => {
        try {
            ...
            set({ certTrusted: isTrusted, certWarningIgnored: config.cert_warning_ignored });
        } catch (error) {
