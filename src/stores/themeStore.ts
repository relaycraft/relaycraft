    ...
    unregisterPluginThemes: (pluginId) => {
        set((state) => {
            ...
            if (activeTheme?.pluginId === pluginId) {
                ...
                Logger.debug(`[ThemeStore] Reverted to default theme (active theme ${activeTheme.id} unloaded)`);
            }
            ...
            if (theme.css) {
                try {
                    ...
                    if (theme.pluginId !== 'system') {
                        ...
                        if (theme.path) {
                            processedCss = cssContent.replace(/url\(['"]?(assets\/[^'"]+)['"]?\)/g, (_match, assetPath) => {
                                const normalizedThemePath = theme.path!.replace(/\\/g, '/');
                                const fullPath = `${normalizedThemePath}/${assetPath}`;
                                const assetUrl = convertFileSrc(fullPath);
                                Logger.debug('[ThemeStore] Resolved asset:', { assetPath, assetUrl });
                                return `url('${assetUrl}')`;
                            });
                        }
