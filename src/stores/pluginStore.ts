...
export const usePluginStore = create<PluginStore>((set, get) => ({
    ...
    fetchPlugins: async () => {
        ...
        try {
            ...
            useNotificationStore.getState().addNotification({
                ...
                message: i18n.t('plugins.notifications.load_failed_msg', { error: String(error) }),
                type: 'error',
                category: 'plugin',
                priority: 'high',
                source: 'System'
    ...
    togglePlugin: async (id, enabled) => {
        try {
            ...
            useNotificationStore.getState().addNotification({
                title: enabled ? i18n.t('plugins.notifications.enabled_title') : i18n.t('plugins.notifications.disabled_title'),
                message: enabled ? i18n.t('plugins.notifications.enabled_msg', { name: pluginName }) : i18n.t('plugins.notifications.disabled_msg', { name: pluginName }),
                type: 'success',
                category: 'plugin',
                priority: 'normal',
            ...
            useNotificationStore.getState().addNotification({
                ...
                message: i18n.t('plugins.notifications.toggle_failed_msg', {
                    action: enabled ? i18n.t('common.enable', { defaultValue: 'enable' }) : i18n.t('common.disable', { defaultValue: 'disable' }),
                    id,
                    error: String(error)
                }),
                type: 'error',
    ...
    fetchMarketPlugins: async (type: 'plugin' | 'theme') => {
        ...
        try {
            const index = await invoke<RegistryIndex>('plugin_market_fetch', { marketType: type });
            if (type === 'theme') {
    ...
    fetchCachedMarketPlugins: async (type: 'plugin' | 'theme') => {
        try {
            const index = await invoke<RegistryIndex>('plugin_market_load_cache', { marketType: type });
            if (type === 'theme') {
    ...
    installPluginLocal: async (path: string) => {
        try {
            ...
            if (errorMessage.includes('Invalid plugin.json')) {
                ...
                message = i18n.t('plugins.errors.invalid_format_msg', { error: errorMessage });
    ...
    uninstallPlugin: async (id: string) => {
        try {
            ...
            useNotificationStore.getState().addNotification({
                ...
                message: i18n.t('plugins.notifications.uninstall_success_msg', { name: pluginName }),
                type: 'success',
                category: 'plugin',
                priority: 'normal',
                source: `Plugin: ${pluginName}`
            ...
            useUIStore.getState().showConfirm({
                ...
                message: i18n.t('plugins.notifications.uninstall_failed_msg', { id, error: errorMessage }),
                variant: 'danger',
                confirmLabel: i18n.t('common.ok', { defaultValue: 'OK' }),
                onConfirm: () => { }
            });
