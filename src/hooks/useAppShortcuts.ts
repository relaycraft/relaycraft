...
export function useAppShortcuts() {
    ...
    useEffect(() => {
        const handleKeyDown = async (e: KeyboardEvent) => {
            ...
            if (isCmdOrCtrl && /^[1-9]$/.test(e.key)) {
                ...
                const allTabs = ['traffic', 'composer', 'rules', 'scripts', ...pluginPages.map(p => p.id)];
            ...
            if (isCmdOrCtrl && e.key.toLowerCase() === 'e') {
                if (selectedFlow) {
                    ...
                    notify.success(t('traffic.context_menu.edit_composer'), { toastOnly: true });
                }
                return;
            }
    ...
    }, [selectedFlow, running, activeTab, t]); // Dependencies updated
}
